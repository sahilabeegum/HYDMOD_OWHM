      MODULE GWFMNW2MODULE
        USE TABLEFILE_INTERFACE,ONLY:TABFILETYPECHARIDX                 !seb
        ...
        CHARACTER(LEN=20),SAVE, DIMENSION(:),POINTER,CONTIGUOUS::WELLID
        CHARACTER(LEN=16),SAVE, DIMENSION(:),POINTER,CONTIGUOUS::MNWAUX
        INTEGER, SAVE,    DIMENSION(:,:),    POINTER,CONTIGUOUS::LIMQ
        DOUBLE PRECISION, SAVE,DIMENSION(:,:),POINTER,CONTIGUOUS::MNW2
        DOUBLE PRECISION, SAVE,DIMENSION(:,:),POINTER,CONTIGUOUS::MNWNOD
        DOUBLE PRECISION, SAVE,DIMENSION(:,:),POINTER,CONTIGUOUS::MNWINT
        DOUBLE PRECISION, SAVE,DIMENSION(:,:,:),POINTER,CONTIGUOUS::
     +                                                  CapTable
        TYPE(TABFILETYPECHARIDX),POINTER,SAVE:: MNW2TABFILE             !seb ADDED TABFILE SUPPORT FROM TABFILE_INTERFACE
      TYPE GWFMNWTYPE
        INTEGER,POINTER  ::NMNW2,MNWMAX,NMNWVL,IWL2CB,MNWPRNT
        INTEGER,POINTER  ::NODTOT,INTTOT,NTOTNOD
        DOUBLE PRECISION, POINTER :: SMALL
        CHARACTER(LEN=20), DIMENSION(:),   POINTER,CONTIGUOUS::WELLID
        CHARACTER(LEN=16), DIMENSION(:),   POINTER,CONTIGUOUS::MNWAUX
        INTEGER,    DIMENSION(:,:),     POINTER,CONTIGUOUS ::LIMQ
        DOUBLE PRECISION,  DIMENSION(:,:), POINTER,CONTIGUOUS::MNW2
        DOUBLE PRECISION,  DIMENSION(:,:), POINTER,CONTIGUOUS::MNWNOD
        DOUBLE PRECISION,  DIMENSION(:,:), POINTER,CONTIGUOUS::MNWINT
        DOUBLE PRECISION,  DIMENSION(:,:,:),POINTER,CONTIGUOUS::CapTable
        TYPE(TABFILETYPECHARIDX),POINTER:: MNW2TABFILE                  !seb ADDED TABFILE SUPPORT FROM TABFILE_INTERFACE
      END TYPE
...
!rth------Revisions by Lenny Konikow 12/26/2012
C     ******************************************************************
C
C     SPECIFICATIONS:
C     ------------------------------------------------------------------
...
C-LFK     2                       CapTable,SMALL,NTOTNOD,WELLID
     2                    CapTable,SMALL,NTOTNOD,WELLID,LIMQ,MNW2TABFILE!seb ADDED MNW2TABFILE
...
C---LFK--modify dates & version
    1 format(/,1x,'MNW2 -- MULTI-NODE WELL 2 PACKAGE, VERSION 7.2,',
     +' 01/02/2013.',/,4X,'INPUT READ FROM UNIT ',i3)
      NMNW2=0
      ntotnod=0
c-lfk-Dec 2012
      nodtot=0
...
C3------CELL-BY-CELL FLOW TERMS, AND PRINT FLAG
      ...
      CALL TABFILEPARSE(IN,IOUT,LINE,MNW2TABFILE)
      CALL TABFILELINKS(IN,IOUT,LINE,MNW2TABFILE)
      !
      LLOC=1
...
C7------SAVE POINTERS TO DATA AND RETURN.
     ...
     +                      Iulmg,Iugmg,Iupcgn,igwtunit,Iunwt,IGRID)
C     ******************************************************************
...
C     ------------------------------------------------------------------
     ...
     1                       IBOUND,LAYCBD
      ...
      USE GWFMNW2MODULE, ONLY:NMNW2,MNWMAX,NMNWVL,IWL2CB,MNWPRNT,
     1                       NODTOT,INTTOT,MNWAUX,MNW2,MNWNOD,MNWINT,
     2                       CapTable,SMALL,WELLID,NTOTNOD,MNW2TABFILE  !seb ADDED TABFILE
...
C     ------------------------------------------------------------------
     ...
     & PUMPLAY,PUMPROW,PUMPCOL,PUMPLOC,PPFLAG,PUMPCAP
      INTEGER Iusip,Iude4,Iusor,Iupcg,Iulmg,Iugmg,Iupcgn,igwtunit,
     & Iunwt,IGRID
...
c        11   = PUMPLOC   WAS PREVIOUSLY DEFINED AS Cprime
...
c  initialize CapFlag2 at beginning of simulation
      mnw2(27,MNWID)=0
c
      IF(LSTCHK(3) .AND. MNWPRNT>-1) THEN
        write(iout,*) 
        write(iout,*) 
...
c     save IR and IC to check vs the subsequent nodes
            ...
            ELSE
              IF((IR.NE.IRlast).OR.(IC.NE.IClast)) THEN
                IF(LSTCHK(1)) THEN
                  write(iout,*) '***ERROR*** Row,Col must be constant ',
     &            'in a vertical well (NNODES < 0)'
                ENDIF
...
c     until ZDP (and ZPL) are set for the well)
                ...
                ELSE
                 IF(LSTCHK(1)) THEN
                   WRITE(iout,*)'***ERROR*** MNW2 screen in no-flow',
     1               ' node'
                 ENDIF
                 STOP 'MNW2 - screen in no-flow node' 
                END IF
...
c        if itmp less than zero, reuse data. print message and return.
        IF(MNW2TABFILE%NTAB.GT.0) THEN                                  !seb IF USING TABFILES UPDATE TABINFORMATION
          DO IW=1,MNWMAX
            IF(MNW2(1,IW).EQ.1) THEN
              ...
              IF(TABFOUND>0)THEN
                MNW2(5,IW)=Qdes
                firstnode=MNW2(4,MNWID)
                lastnode=IDINT(MNW2(4,MNWID) + ABS(MNW2(2,MNWID)) - 1D0)
                MNWNOD(4,firstnode:lastnode)=0.0                        !seb ZERO OUT ALL NODES AND PUT DESIRED INTO FIRST NODE
...
c
     ...
     & kper
        ...
        ENDIF
        do iread=1,ITMP
          TABFOUND=0                                              !seb FLAG TO INDICATE IF WELL WAS DEFINED WITH A TABFILE
c  read data set 4a
...
C-LFK
       ...
       ENDIF
         ...
         !
         IF(MNW2TABFILE%NTAB.GT.0) CALL MNW2TABFILE2QDES(MNW2TABFILE,   !seb IF TABFILES THEN CHECK IF QDES IS RELATED TO 1 TABFILE. IF IT IS THEN UPDATE IT WITH TABIFORMATION
     +                                              MNWID,QDES,TABFOUND)
         !
...
c          put total Qdes in first node, allow it to go from there
         lastnode=IDINT( MNW2(4,MNWID) + ABS(MNW2(2,MNWID)) - 1D0 )     !seb
         MNWNOD(4,firstnode:lastnode)=0.0                               !seb ZERO OUT ALL NODES AND PUT DESIRED INTO FIRST NODE
         ...
         IF(TABFOUND.EQ.0 .AND. MNWPRNT>-1)THEN                         !seb ADDED ' [Q Derived from Tabfile]'
           IF(LSTCHK(3)) write(iout,2112) WELLNAME,Qdes,kper
         ELSE
           IF(LSTCHK(3)) write(iout,2113) WELLNAME,Qdes,kper
         END IF
 2112 FORMAT('MNW2 Well ', A20,' active, desired Q =',
     + 1pe12.4,' for stress period ',I4)
 2113 FORMAT('MNW2 Well ', A20,' active, desired Q =',
     + 1pe12.4,' for stress period ',I4,' [Q Derived from Tabfile]')
         if(PUMPCAP.GT.0) then
c  if Qdes is not negative for this stress period, do not apply pump capacity restraints
           if(Qdes.GT.0.d0) then                                        !seb (Qdes.GE.0.d0) CHANGED TO (Qdes.GT.0.d0)
              MNW2(25,MNWID)=0                                          !Initialize CapFlag2
              mnw2(27,MNWID)=0
           else
             IF(Qdes.EQ.0D0)THEN                                        !seb ALLOW HOLDING OF CAPMULT FOR Qdes=0 BUT DO NOT ENABLE CAPACITY CONTSTRAINTS TO FACILITATE LINK TO FMP
              MNW2(25,MNWID)=0
              MNW2(27,MNWID)=0                                          !Initialize CapFlag2
             ELSE
              MNW2(25,MNWID)=1
...
c1------if number of wells <= 0 then return.
      ...
      !
      IF(MNW2TABFILE%NTAB.GT.0) THEN                                    !seb IF USING TABFILES UPDATE TABINFORMATION
        MNW2TABFILE%TAB%USEVAL=.FALSE.                                  !TELL TABFILEINTERP TO SEARCH FOR NEW TIME AND THEN START REUSING VALUES
        IF(MNW2TABFILE%TABPRINT)THEN                                        !PRINT OUT HEADER TO IOUT
       WRITE(IOUT,'(/A,1PG15.9)')'MNW2 TABFILE PRINT OUT FOR TIME ',
     +                           TOTIM
        ...
        DO IW=1,MNWMAX
          IF(MNW2(1,IW).EQ.1) THEN
            ...
            IF(TABFOUND>0)THEN
              MNW2(5,IW)=Qdes
              firstnode=MNW2(4,IW)
              lastnode=IDINT(MNW2(4,IW) + ABS(MNW2(2,IW)) - 1D0)
              ...
              IF(MNW2TABFILE%TABPRINT)THEN
                ...
                WRITE(IOUT,'(A,1PG15.9,ES15.8,2x,A)')WELLID(IW),
...
c   Loop over nodes in well
            ...
            do INODE=firstnode,lastnode
              ...
              ir=MNWNOD(2,INODE)              
              ic=MNWNOD(3,INODE)              
              if( (Iuupw.EQ.0 .AND. IBOUND(ic,ir,il).ne.0) .OR.
     +        (Iuupw.NE.0 .AND. HNEW(IC,IR,IL).GT.BOTM(IC,IR,LBOTM(IL))
     +                    .AND. IBOUND(ic,ir,il).ne.0 ) 
     +                                                            ) THEN!SCOTT SHOULD THERE BE AN ADDITION CHECK FOR HNEW<BOT FOR AN INACTIVE CELL
                csum  = csum  + MNWNOD(14,INODE)
                hhnew=hnew(ic,ir,il)
                chsum = chsum + MNWNOD(14,INODE)*hhnew
                qact  = qact  + MNWNOD(4,INODE)
              !else
              !  qact  = 0.0000D0                                        ! seb SHOULD NOT ZERO OUT qact otherwise it will not be correct when inactive layer is inbetween two active layers
              end if            
...
c                mnw2(18,iw) = Qpot
                mnw2(30,iw) = Qpot
                MNWNOD(4,firstnode)=Qpot                                !SCOTT SEEMS STRANGE NOT TO SET THE REST OF THE NODES TO ZERO
            else
...
c  Specify Q and solve for head;  add Q to RHS accumulator.
              ...
              IF ( Iuupw.EQ.0 ) THEN
ccrth                qact = ( hlim - hhnew) * cond !adjusted as before for other solvers --rth Not used by rgn????
                MNWNOD(4,INODE) = qact
              ELSE
...
c -----print the header for individual rates if requested(IWL2CB<0).
      ioch = 0
      if( IWL2CB.lt.0 .and. icbcfl.ne.0  .AND. MNWPRNT>-1) then
        IF(LSTCHK(3)) THEN
...
c  active well check
         if(MNW2(1,iw).eq.1) then
          ...
          DO INODE=firstnode,lastnode
              ...
              IR=MNWNOD(2,INODE)              
              IC=MNWNOD(3,INODE)  
              IF(IBOUND(IC,IR,IL).EQ.0) CONTINUE
              IF(HNEW(IC,IR,IL).EQ.HDRY)CONTINUE
              Bottom = BOTM(IC,IR,LBOTM(IL))
              IF(HNEW(IC,IR,IL).LE.Bottom.AND.Iuupw.NE.0)CONTINUE
...
c   Loop over nodes in well
          do INODE=firstnode,lastnode
            ...
            ic=MNWNOD(3,INODE)              
            nd=INODE-firstnode+1           
            DryTest = Hnew(ic,ir,il) - Hdry                             !Scott SHOULD THIS BE AN IBOUND CHECK? SEE NEW NEXT LINE
     ...
     +                                                       DryTest=0D0!seb ADDED CHECK FOR NWT DRY CELL WHERE HNEW<BOT  SCOTT THIS NEEDS TO BE DOUBLE CHECKED BUT BELOW IT FLAGS A WRITE SAYING THIS IS TRUE, BUT NO STATEMENT EXISTED FOR IT TO HAPPEN
...
!            else  !commented out and added "end if" seb
!              iweldry=0
!            endif
            q = MNWNOD(4,INODE)
c  check to write message that inode was deactivated this time step
                if(mnwprnt.gt.1.and.mnwnod(14,inode).eq.0.0) then
                  IF(HNEW(IC,IR,IL).LT.BOTM(IC,IR,LBOTM(IL))
     +                                 .AND.HNEW(IC,IR,IL).NE.HDRY) THEN
      IF(LSTCHK(2) .AND. MNWPRNT>-1) THEN
...
c    Report all wells with production less than the desired rate......
            if(ibound(ic,ir,il).NE.0 .or.ABS(DryTest).GT.verysmall) then!If it is active or dry? seb I AGREE< LOOKS LIKE IT SHOULD NOT be .NE.0 IF CELL IS DRY
              if(MNW2(6,iw).NE.0) then
                hlim = mnw2(7,iw)
              else
ccrth                hlim = 0.0D0   !This seems very bad. Why set it to zero?
                hlim = -9.99D50   !Reset to a very low elevation seb set from D6 to D50
              end if
c
              hwell = mnw2(17,iw)
...
!                 IF ( MNW2(6,iw).NE.0 )THEN     !seb commented out redundant block since its already set in the preceding block of code
!                   hlim = MNW2(7,iw)
!                 ELSE
!                   hlim = hwell
!                 END IF
...
c -----print the individual rates if requested(IWL2CB<0).
              if( ioch.eq.1 ) then
              IF ( Iuupw.EQ.0 ) then
!               q=MNWNOD(4,INODE)   !RGN  seb commented line cause its already set from above
               hcell=hnew(ic,ir,il)
              ENDIF
              nd=INODE-firstnode+1
c   If no seepage face in cell, don't print seepage elev.
              if(MNWNOD(15,INODE).EQ.hwell.or.
     +           MNWNOD(15,INODE).eq.Hdry.or.abs(MNW2(2,IW)).EQ.1) then  !lfk 10/29/2012
                IF(LSTCHK(3) .AND. MNWPRNT>-1) THEN
...
c-lfk  10/10/2012 & 10/29/2012
     ...
     +           MNWNOD(15,INODE).ne.Hdry.and.numnddel.ne.0) then !lfk
                 iseepflg=1
              end if
c--lfk
C--LFK  Nov. 2012 refine printout for cause of reduced Qdes
cc             if (mnw2(27,iw).ne.0.and.(qnet-mnw2(29,iw)).lt.small) then
cc                    write(iout,*) WELLID(iw),' Qdes has been updated to',
cc     & qnet,' because of Pump Capacity restraint'
cc                else if (mnw2(6,iw).NE.0) then
cc                    write(iout,*) WELLID(iw),' Qdes has been updated to',
cc     & qnet,' because of Hlim constraint'
cc                else
cc                    write(iout,*) WELLID(iw),' Qnet = ',qnet,' < Qdes'
cc                end if
            IF(LSTCHK(3)) THEN
            write(iout,'(3A,2(G16.8,A))') ' MNW2 WELL: ',WELLID(iw),
     +      ' Qnet = ',qnet, ' < ',Qdes,' = Qdes'                   !seb ADDED WHAT QDES ACTUALLY IS
            ENDIF
!C--LFK  Nov. 2012 refine printout for cause of reduced Qdes
...
C--LFK
            ...
            R2=LIMQ(2,IW)
            ISEEPFLG=LIMQ(3,IW)
C--LFK  Nov. 2012 refine printout for cause of reduced Qdes  !MOVED FROM PREVIOUS LOCATION 12 lines up
             if (mnw2(27,iw).ne.0.and.(qnet-mnw2(29,iw)).lt.small) then
                  R1=1
             END IF     
            if(mnw2(6,iw).NE.0.AND.(abs(qnet)-abs(mnw2(30,iw))).lt.small
     +        ) then  
            ...
            END IF
C
            IF(LSTCHK(3)) THEN                                          ! seb MERGED MULTIPLE LSTCHK CALLS INTO ONE LARGE OUTER ONE
...
C     FIND HORIZONTAL ANISOTROPY, THE RATIO Ky/Kx
         ...
         ELSE
          KHANI=NINT(-CHANI(IZ))
!          IF(ITRSS.NE.0) THEN    !seb need to comment to fix
            AH = HANI(IX,IY,KHANI)
!          ELSE
!            AH = HANI(1,1,1)
!          END IF
...
c
          ...
          thick = upper - bot
c   set thickness / conductance to 0 if cell is dry
          if( (hnew(ix,iy,iz)-Hdry )**2 .lt. verysmall )                !SCOTT THIS WILL NEVER HAPPEN IN UPW
     &          thick = 0.0000000000D0
          if (thick<0D0) thick = 0.0000000000D0
          Txx = TempKX * thick
          if( Txx .lt.verysmall ) Txx = 0.000000000000000D0
          Tyy = Txx * AH
         endif
...
c-----if the cell is inactive or specified then bypass processing.
         ...
         if(Iuupw.NE.0.AND.hnew(ix,iy,iz).LE.BOTM(IX,IY,LBOTM(IZ))) 
     +                                                         irecalc=0!seb FIX TO ENSURE THAT DRY CELLS OR THICK=0 DO NOT GET PROCESSED
...
c   calculate total length of borehole within cell
                ...
                if(LOSSTYPE.EQ.4) then
                  if(totlength.gt.0D0) then
                    lengthratio=lengthint/totlength                     !SCOTT DOES NOT MAKE SENSE FIRST TIME THROUGH IS 1 ALSO MANUAL SAYS THAT THIS IS NOT DONE
...
C
     ...
     + Ploss,B,Rskin,Kskin,C,Cf,Q,thck,T,Tskin,Skin,h,dc,TOP,BOT,HED,SF
      INTERFACE
...
C-LFK
      ISEGFLG=0
C      
      if( rw.lt.verysmall .or. Txx.lt.verysmall .or. Tyy.lt.verysmall ) !SCOTT THIS DOES NOT MAKE SENSE rw is undefine for LOSSTYPE=NONE
     +  then
...
c       if ro/rw is <1, 'A' term will be negative.  Warn user and cut off flow from this node
     ...
     +      '***WARNING*** Ro/Rw < 1, CWC set = 0.0 for well ',WELLNAME
        ...
        A = log(ro/rw) / Tpi2                                                     !SCOTT Rw is undefined when LOSSTYPE=NONE
...
C
      ...
      IF ( LAYHDT(L1).GT.0 .AND. Iuupw.NE.0 ) THEN
...
C-LFK   initialize MNWNOD(15,IW) TO HIGH VALUE
      INODE = MNW2(4,IW)                   !THis Inititialization commented out by Lenny Dec-2012??
      iseepflg=0
      MNWNOD(15,INODE) = 1.0D31            !THis Inititialization commented out by Lenny Dec-2012??
      qdes = mnw2(5,iw)
      Qsmall = small*abs(qdes)
         qoff=0.0
         qon=0.0
...
c   Loop over nodes in well
     ...
     +                                                            ) THEN
                csum  = csum  + MNWNOD(14,INODE)
                chsum = chsum + MNWNOD(14,INODE)*hnew(ic,ir,il)         !SCOTT IF hnew<botm this creates ficticious flow, but if you view HNEW as a pressure then it will be sucking on Hwel in the calc hwell = ( qdes + chsum ) / csum  where csum contains HNEW
                qact  = qact  + MNWNOD(4,INODE)
              !else
              !  qact  = 0.0000D0                                        ! seb SHOULD NOT ZERO OUT qact otherwise it will not be correct when inactive layer is inbetween two active layers
              end if            
...
c
             IF(hnew(ic,ir,il).NE.HDRY)  THEN                           !seb broke into two if statements, one that checks for HDRY and the other checks for NWT style not dry
...
c-LFK--use 'Skin' as a flag to denote calc. is for a segment
             Skin=-999.0                                                !SCOTT THIS IS BAD, SHOULD JUST DOUBLE VARIABLES ON ENTRY
c
...
C
      DO 1 I=1,NS
       PP=XLN2*DBLE(I)/TD
       IF ( pp.GT.1.0e-14 ) THEN                                        !seb Changed from (pp.LT.1.0e-14)
         Q0=DSQRT(PP)
       ELSE
         Q0=0.0
       END IF
...
C  Deallocate MNW MEMORY
      USE GWFMNW2MODULE
C
        DEALLOCATE(GWFMNWDAT(IGRID)%NMNW2)                              !seb ADDED TO FOLLOW STATEMENTS: GWFMNWDAT(IGRID)%
        ...
        DEALLOCATE(GWFMNWDAT(IGRID)%MNW2TABFILE)                        !seb
C
c-lfk
...
C NULLIFY THE LOCAL POINTERS
        IF(IGRID.EQ.1)THEN
          NMNW2       =>NULL()                       !seb ADDED TO FOLLOW STATEMENTS: GWFMNWDAT(IGRID)%
          MNWMAX      =>NULL()
          NMNWVL      =>NULL()
          IWL2CB      =>NULL()
          MNWPRNT     =>NULL()
...
C
        ...
        MNW2TABFILE=>GWFMNWDAT(IGRID)%MNW2TABFILE                       !seb
        LIMQ=>GWFMNWDAT(IGRID)%LIMQ                                     !seb MISSING POINTER REFERANCE
...
C
        ...
        GWFMNWDAT(IGRID)%MNW2TABFILE=>MNW2TABFILE                       !seb
        GWFMNWDAT(IGRID)%LIMQ=>LIMQ                                     !seb MISSING POINTER REFERANCE
C
      RETURN
      END
