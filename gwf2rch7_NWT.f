      MODULE GWFRCHMODULE
        ...
        REAL,    SAVE,   DIMENSION(:,:),  POINTER,CONTIGUOUS::RECH
        INTEGER, SAVE,   DIMENSION(:,:),  POINTER,CONTIGUOUS::IRCH
      TYPE GWFRCHTYPE
        INTEGER,  POINTER                 ::NRCHOP,IRCHCB
        INTEGER,  POINTER                 ::NPRCH,IRCHPF
        REAL,       DIMENSION(:,:),  POINTER,CONTIGUOUS::RECH
        INTEGER,    DIMENSION(:,:),  POINTER,CONTIGUOUS::IRCH
      END TYPE
...
C     ------------------------------------------------------------------
     ...
     +                      HNEW,BOTM,LBOTM,LAYHDT                      !seb ADDED HNEW, BOTM, and LBOTM
...
C3A-----RIGHT-HAND-SIDE.
        IF(IBOUND(IC,IR,1).GT.0) THEN
          HHNEW=REAL(HNEW(IC,IR,1),KIND(HHNEW))
          BOT=BOTM(IC,IR,LBOTM(1))
          IF(LAYHDT(1).GT.0 .AND. HHNEW.LT.BOT) GO TO 10               ! seb ADDED FIX FOR NWT DRY CELLS
...
C4A-----RIGHT-HAND-SIDE.
        ...
        IF(IBOUND(IC,IR,IL).GT.0)THEN
          HHNEW=REAL(HNEW(IC,IR,IL),KIND(HHNEW))
          BOT=BOTM(IC,IR,LBOTM(IL))
          IF(LAYHDT(IL).GT.0 .AND. HHNEW.LT.BOT) GO TO 20               ! seb ADDED FIX FOR NWT DRY CELLS
...
C5B-----RIGHT-HAND-SIDE AND MOVE TO NEXT HORIZONTAL LOCATION.
        IF(IBOUND(IC,IR,IL).GT.0) THEN
          HHNEW=REAL(HNEW(IC,IR,IL),KIND(HHNEW))
          BOT=BOTM(IC,IR,LBOTM(IL))
          IF(LAYHDT(IL).GT.0 .AND. HHNEW.LT.BOT) GO TO 28               ! seb ADDED FIX FOR NWT DRY CELLS
          RHS(IC,IR,IL)=RHS(IC,IR,IL)-RECH(IC,IR)
          GO TO 30
        END IF
   28   CONTINUE
