...
C4------ALLOCATE SPACE FOR THE BNDS ARRAY.
      ...
      !PRINT *,ASSOCIATED(GHBDATA)
      !READ (*,*)
      !PRINT *, GHBDATA(1)%VAL(1)
      !READ (*,*)
      ...
      DO I=1,MXBND
        ...
        IF(GHBTABFILE%NTAB.GT.0)THEN
                      PRINT *,ALLOCATED(GHBDATA(I)%TABNAM)

            IF (.NOT.ALLOCATED(GHBDATA(I)%TABNAM)) THEN
                      ALLOCATE(GHBDATA(I)%TABNAM   )

            ENDIF
                      PRINT *,I, MXBND, ALLOCATED(GHBDATA(I)%TSFAC)
                      READ (*,*)
            IF (.NOT.ALLOCATED(GHBDATA(I)%TSFAC)) THEN
...
C
        ...
        !PRINT *, GWFGHBDAT(IGRID)%GHBDATA%TSFAC
        !READ (*,*)
C
