     ...
     1                             IOHUFHDS,IOHUFFLWS
        ...
        CHARACTER(LEN=10),SAVE,POINTER, DIMENSION(:),CONTIGUOUS::HGUNAM
        INTEGER, SAVE,   POINTER, DIMENSION(:)    ,CONTIGUOUS::LTHUF
        INTEGER, SAVE,   POINTER, DIMENSION(:)    ,CONTIGUOUS::LAYWT
        INTEGER, SAVE,   POINTER, DIMENSION(:,:)  ,CONTIGUOUS::IHGUFLG
        REAL,    SAVE,   POINTER, DIMENSION(:)    ,CONTIGUOUS::HGUHANI
        REAL,    SAVE,   POINTER, DIMENSION(:)    ,CONTIGUOUS::HGUVANI
        REAL,    SAVE,   POINTER, DIMENSION(:)    ,CONTIGUOUS::HUFHK
        REAL,    SAVE,   POINTER, DIMENSION(:)    ,CONTIGUOUS::HUFVK
        REAL,    SAVE,   POINTER, DIMENSION(:)    ,CONTIGUOUS::HUFSS
        REAL,    SAVE,   POINTER, DIMENSION(:)    ,CONTIGUOUS::HUFSY
        REAL,    SAVE,   POINTER, DIMENSION(:)    ,CONTIGUOUS::HUFHANI
        REAL,    SAVE,   POINTER, DIMENSION(:)    ,CONTIGUOUS::HUFKDEP
        REAL,    SAVE,   POINTER, DIMENSION(:,:)  ,CONTIGUOUS::GS,SC2HUF
        REAL,    SAVE,   POINTER, DIMENSION(:,:,:),CONTIGUOUS::VKAH
        REAL,    SAVE,   POINTER, DIMENSION(:,:,:),CONTIGUOUS::SC1
        REAL,    SAVE,   POINTER, DIMENSION(:,:,:),CONTIGUOUS::WETDRY
        REAL,    SAVE,   POINTER, DIMENSION(:,:,:),CONTIGUOUS::HK
        REAL,    SAVE,   POINTER, DIMENSION(:,:,:),CONTIGUOUS::HKCC
        REAL,    SAVE,   POINTER, DIMENSION(:,:,:),CONTIGUOUS::HUFTMP
        REAL,    SAVE,   POINTER, DIMENSION(:,:,:),CONTIGUOUS::VDHD
        REAL,    SAVE,   POINTER, DIMENSION(:,:,:,:),CONTIGUOUS::HUFTHK
        REAL,    SAVE,   POINTER, DIMENSION(:,:,:,:),CONTIGUOUS::VDHT
        REAL,    SAVE,   POINTER, DIMENSION(:,:,:,:),CONTIGUOUS::A9
      TYPE GWFHUFTYPE
        INTEGER, POINTER ::IHUFCB,NHUF,NPHUF,IWETIT,IHDWET,
     1                     IOHUFHDS,IOHUFFLWS 
        REAL, POINTER    ::WETFCT
        CHARACTER(LEN=10), POINTER, DIMENSION(:),CONTIGUOUS::HGUNAM    
        INTEGER,   POINTER, DIMENSION(:)    ,CONTIGUOUS::LTHUF
        INTEGER,   POINTER, DIMENSION(:)    ,CONTIGUOUS::LAYWT
        INTEGER,   POINTER, DIMENSION(:,:)  ,CONTIGUOUS::IHGUFLG
        REAL,      POINTER, DIMENSION(:)    ,CONTIGUOUS::HGUHANI
        REAL,      POINTER, DIMENSION(:)    ,CONTIGUOUS::HGUVANI
        REAL,      POINTER, DIMENSION(:)    ,CONTIGUOUS::HUFHK
        REAL,      POINTER, DIMENSION(:)    ,CONTIGUOUS::HUFVK
        REAL,      POINTER, DIMENSION(:)    ,CONTIGUOUS::HUFSS
        REAL,      POINTER, DIMENSION(:)    ,CONTIGUOUS::HUFSY
        REAL,      POINTER, DIMENSION(:)    ,CONTIGUOUS::HUFHANI
        REAL,      POINTER, DIMENSION(:)    ,CONTIGUOUS::HUFKDEP
        REAL,      POINTER, DIMENSION(:,:)  ,CONTIGUOUS::GS,SC2HUF
        REAL,      POINTER, DIMENSION(:,:,:),CONTIGUOUS::VKAH
        REAL,      POINTER, DIMENSION(:,:,:),CONTIGUOUS::SC1
        REAL,      POINTER, DIMENSION(:,:,:),CONTIGUOUS::WETDRY
        REAL,      POINTER, DIMENSION(:,:,:),CONTIGUOUS::HK
        REAL,      POINTER, DIMENSION(:,:,:),CONTIGUOUS::HKCC
        REAL,      POINTER, DIMENSION(:,:,:),CONTIGUOUS::HUFTMP
        REAL,      POINTER, DIMENSION(:,:,:),CONTIGUOUS::VDHD
        REAL,      POINTER, DIMENSION(:,:,:,:),CONTIGUOUS::HUFTHK
        REAL,      POINTER, DIMENSION(:,:,:,:),CONTIGUOUS::VDHT
        REAL,      POINTER, DIMENSION(:,:,:,:),CONTIGUOUS::A9
      END TYPE
...
C        SPECIFICATIONS:
C     ------------------------------------------------------------------
      USE GLOBAL,      ONLY:NCOL,NROW,NLAY,NBOTM,BOTM,ITRSS,LAYHDT,
     1                      LAYHDS,IOUT,ISSFLG,NPER,IBOUND,LBOTM,HNEW !swm: added NBOTM and HNEW
...
C1------IDENTIFY PACKAGE
    ...
    1 FORMAT(1X,/1X,'HUF2 -- HYDROGEOLOGIC-UNIT FLOW PACKAGE,
     & VERSION 7.1, 05/31/2013',/,' INPUT READ FROM UNIT',I3,/)
C    Rev 7.1 include Modifications from Wolfgang Schmid, Lenny Konikow, Scott Polinski, and Randy Hanson
C2------CHECK IF LGR IS ACTIVE, IF SO STOP.=> this stop disabled by rth
!      IF(ILGR .NE. 0)THEN 
!        IF(LSTCHK(1)) THEN
!          WRITE(IOUT,*)
!     1       'THE HUF PACKAGE CANNOT BE USED WHEN RUNNING LGR' 
!          WRITE(IOUT,*)
!     1       'RUN LGR FIRST TO PRODUCE THE FILES NEEDED BY HUF'
!        ENDIF
!        CALL USTOP(' ')
!      ENDIF 
C2A------READ FIRST RECORD AND WRITE
        ...
        DO  I=1,NROW
          DO  J=1,NCOL
            ...
            UPLAY: DO WHILE ( KK.LE.NLAY )
              ...
              END IF
            END DO UPLAY
C LFK prevent LTHUF array subscript of K=0
          IF (K.GT.0)THEN
            IF (LTHUF(K).NE.0) THEN           
...
cswm:  NOTE: VK within layer calc. is not working correctly.  Instead
cswm:        use HUF's VKAH which is vert K between layers.
...
C           GET THICKNESS OF UNIT IN THIS LAYER
     ...
     &                        TOPU,THCKU,THCK,ATPU,ABTU)
            ...
            IF(HGUVANI(NU).EQ.0.) THEN
              ...
              IF(HUFVK(NU) .EQ.0.) GOTO 210  !swm: avoid divide by 0
              VK(J,I)=VK(J,I)+THCK/HUFVK(NU)
            ELSE
              MULTKDEP = 1.0
              CALL UHUF7POP(HUFHK,'HK  ',I,J,NU,IOUT)
              CALL UHUF7POP(HUFVK,'VANI',I,J,NU,IOUT)
              ! HUFVK should be set to HGUVANI if no VANI parameter 
              ! is defined for unit NU -- Added next line 1/15/08 ERB
              IF(HUFVK(NU)==0.0) HUFVK(NU) = HGUVANI(NU)    
              !
              CALL UHUF7POP(HUFKDEP,'KDEP',I,J,NU,IOUT)
cswm              IF(HUFKDEP(NU).NE.0.) CALL SGWF2HUF7KDEP(HUFKDEP(NU),TOPU,
cswm     &                                                 BOTU,GS(J,I),
cswm     &                                                 MULTKDEP)
CSWM: USE ADJUSTED VALUES OF TOP AND BOTTOM 
              IF(HUFKDEP(NU).NE.0.) CALL SGWF2HUF7KDEP(HUFKDEP(NU),ATPU,
     &                                                 ABTU,GS(J,I),
     &                                                 MULTKDEP)
              IF(HUFHK(NU) .EQ. 0. .OR. MULTKDEP .EQ. 0.) GOTO 210  !swm: avoid divide by 0
...
C         VK=THICK(LAYER)/VK
      DO 400 I=1,NROW
        DO 500 J=1,NCOL
!swm          IF(IBOUND(J,I,KL).NE.0.0) VK(J,I)=(BOTM(J,I,LBOTM(KL)-1) -
!swm     &                                     BOTM(J,I,LBOTM(KL)))/VK(J,I)
          IF(IBOUND(J,I,KL).NE.0 .AND. VK(J,I).NE.0.) 
     &      VK(J,I)=(BOTM(J,I,LBOTM(KL)-1) -BOTM(J,I,LBOTM(KL)))/VK(J,I)
  500   CONTINUE
  400 CONTINUE
C       END COL LOOP
C     END ROW LOOP
...
C        SPECIFICATIONS:
C     ------------------------------------------------------------------
      USE GLOBAL,      ONLY:LBOTM,DEFAULT_KIND
      USE GWFHUFMODULE,ONLY:LTHUF
C
      DOUBLE PRECISION HNEW
      REAL(DEFAULT_KIND)::CR,CC                                         !seb PREVIOUSLY IMPLICITLY DEFINED AS REAL
C
      DIMENSION HNEW(NCOL,NROW,NLAY),IBOUND(NCOL,NROW,NLAY),
...
C     
      ...
      REAL(DEFAULT_KIND):: CV                                           !seb PREVIOUSLY IMPLICITLY DEFINED AS REAL
...
C     ------------------------------------------------------------------
      ...
      USE GWFHUFMODULE,ONLY:LTHUF,LAYWT
      DOUBLE PRECISION HNEW,HHD,BBOT,TTOP
      REAL(DEFAULT_KIND)::CC                                            !seb PREVIOUSLY IMPLICITLY DEFINED AS REAL
C
      DIMENSION HNEW(NCOL,NROW,NLAY),IBOUND(NCOL,NROW,NLAY),
     & CC(NCOL,NROW,NLAY),
...
C     ------------------------------------------------------------------
      ...
      ISS=ISSFLG(KPER)
C
C1------CHECK IF LGR IS ACTIVE, IF SO STOP. =>Disabled stop by rth
!      IF(ILGR .NE. 0)THEN 
!        IF(LSTCHK(1)) THEN
!          WRITE(IOUT,*)
!     1       'THE HUF PACKAGE CANNOT BE USED WHEN RUNNING LGR' 
...
!        CALL USTOP(' ')
!      ENDIF 
C2------IF ANY LAYER IS CONVERTIBLE REPOPULATE ARRAYS AND CALCULATE
...
C
C2A------IF THE STRESS PERIOD IS TRANSIENT, ADD STORAGE TO HCOF AND RHS
C
      IF(ISS.EQ.0) THEN
         TLED=ONE/DELT
         DO 200 K=1,NLAY
...
C2------IF STEADY STATE, STORAGE TERM IS ZERO
      IF(ISS.NE.0) THEN                                                 !seb ADDED PRINT OUT OF BUDGET EVEN WHEN STEADY STATE OCCURS
...
C        SPECIFICATIONS:
C     ------------------------------------------------------------------
      USE GLOBAL,      ONLY: DEFAULT_KIND,LSTCHK
      CHARACTER*16 TEXT(3)
      REAL(DEFAULT_KIND)::CV
...
C1------WRITE TWO UNFORMATTED RECORDS IDENTIFYING DATA.
    ...
    2   FORMAT(1X,'SGWF2HUF7FLOT SAVING "',A16,'" ON UNIT',I4,
     &       ' AT TIME STEP',I5,', STRESS PERIOD',I3)
C        WRITE(IHUFCB) KSTP,KPER,TEXT(ICNT),NCOL,NROW,NHUF
...
C     ------------------------------------------------------------------
      ...
      REAL LAMBDA,MULTKDEP
      REAL(DEFAULT_KIND):: CV                                           !seb PREVIOUSLY DEFINED AS IMPLICIT REAL
...
C     ------------------------------------------------------------------
      USE GLOBAL, ONLY: DEFAULT_KIND
      REAL(DEFAULT_KIND)::CR,CC,RHS                                     !seb PREVIOUSLY IMPLICITLY DEFINED AS REAL 
