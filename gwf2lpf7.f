      MODULE GWFLPFMODULE
        ...
        INTEGER, SAVE,   POINTER,CONTIGUOUS, DIMENSION(:)     ::LAYTYP
        INTEGER, SAVE,   POINTER,CONTIGUOUS, DIMENSION(:)     ::LAYAVG
        REAL,    SAVE,   POINTER,CONTIGUOUS, DIMENSION(:)     ::CHANI
        INTEGER, SAVE,   POINTER,CONTIGUOUS, DIMENSION(:)     ::LAYVKA
        INTEGER, SAVE,   POINTER,CONTIGUOUS, DIMENSION(:)     ::LAYWET
        INTEGER, SAVE,   POINTER,CONTIGUOUS, DIMENSION(:)     ::LAYSTRT
        INTEGER, SAVE,   POINTER,CONTIGUOUS, DIMENSION(:,:)   ::LAYFLG
        REAL,    SAVE,   POINTER,CONTIGUOUS, DIMENSION(:,:,:) ::VKA
        REAL,    SAVE,   POINTER,CONTIGUOUS, DIMENSION(:,:,:) ::VKCB
        REAL,    SAVE,   POINTER,CONTIGUOUS, DIMENSION(:,:,:) ::SC1
        REAL,    SAVE,   POINTER,CONTIGUOUS, DIMENSION(:,:,:) ::SC2
        REAL,    SAVE,   POINTER,CONTIGUOUS, DIMENSION(:,:,:) ::HANI
        REAL,    SAVE,   POINTER,CONTIGUOUS, DIMENSION(:,:,:) ::WETDRY
        REAL,    SAVE,   POINTER,CONTIGUOUS, DIMENSION(:,:,:) ::HK
      TYPE GWFLPFTYPE
        INTEGER, POINTER ::ILPFCB,IWDFLG,IWETIT,IHDWET
        INTEGER, POINTER ::ISFAC,ICONCV,ITHFLG,NOCVCO,NOVFC
        REAL, POINTER    ::WETFCT
        INTEGER,   POINTER,CONTIGUOUS, DIMENSION(:)     ::LAYTYP
        INTEGER,   POINTER,CONTIGUOUS, DIMENSION(:)     ::LAYAVG
        REAL,      POINTER,CONTIGUOUS, DIMENSION(:)     ::CHANI
        INTEGER,   POINTER,CONTIGUOUS, DIMENSION(:)     ::LAYVKA
        INTEGER,   POINTER,CONTIGUOUS, DIMENSION(:)     ::LAYWET
        INTEGER,   POINTER,CONTIGUOUS, DIMENSION(:)     ::LAYSTRT
        INTEGER,   POINTER,CONTIGUOUS, DIMENSION(:,:)   ::LAYFLG
        REAL,      POINTER,CONTIGUOUS, DIMENSION(:,:,:) ::VKA
        REAL,      POINTER,CONTIGUOUS, DIMENSION(:,:,:) ::VKCB
        REAL,      POINTER,CONTIGUOUS, DIMENSION(:,:,:) ::SC1
        REAL,      POINTER,CONTIGUOUS, DIMENSION(:,:,:) ::SC2
        REAL,      POINTER,CONTIGUOUS, DIMENSION(:,:,:) ::HANI
        REAL,      POINTER,CONTIGUOUS, DIMENSION(:,:,:) ::WETDRY
        REAL,      POINTER,CONTIGUOUS, DIMENSION(:,:,:) ::HK
      END TYPE
...
C5------ALLOCATE MEMORY FOR ARRAYS.
      ...
      END IF
C
C seb ADDED INITIALIZATION OF VARIABLES
      LAYFLG=0
      HK    =0.0
      VKA   =0.0
      VKCB  =0.0
...
C seb added output of final array
      IF(PROPPRINT.NE.0) CALL PRINTARRAY(IGRID,1)
C
C9------RETURN
...
C3-----RETURN
      RETURN
      END
      SUBROUTINE GWF2LPF7FM(KITER,KSTP,KPER,ILGR,IGRID)                 !swm - LGR added by wschmid
C     ******************************************************************
...
C2------FOR EACH LAYER: IF CONVERTIBLE, CALCULATE CONDUCTANCES.
      ...
      KK=K
      IF(LAYTYP(K).NE.0)
     1    CALL SGWF2LPF7HCOND(KK,KITER,KSTP,KPER,ILGR)                !swm - LGR added by wschmid
  100 CONTINUE
      DO 101 K=1,NLAY
      KK=K
      IF(K.NE.NLAY) THEN
...
C5------CALCULATE HOR. CONDUCTANCE(CR AND CC) FOR CONSTANT T LAYERS.
      DO 70 K=1,NLAY
      KK=K
      IF(LAYTYP(K).EQ.0) CALL SGWF2LPF7HCOND(KK,0,0,0,ILGR)        !swm - LGR added by wschmid
   70 CONTINUE
C
...
C
      RETURN
      END
      SUBROUTINE SGWF2LPF7HCOND(K,KITER,KSTP,KPER,ILGR)         !swm - LGR added by wschmid
C     ******************************************************************
...
C2------IF LAYER IS WETTABLE CONVERT DRY CELLS TO WET WHEN APPROPRIATE.
     ...
     2      IHDCNV,NCNVRT,ICNVRT,JCNVRT,ACNVRT,ILGR)             !swm - LGR added by wschmid
C
...
C7------RETURN.
     ...
     1                        ICNVRT,JCNVRT,ACNVRT,ILGR)          !swm - LGR added by wschmid
C
C     ******************************************************************
...
C4------REACHED.
         ...
         ELSE IF (ILGR.NE.0) THEN                !swm - LGR added by wschmid
            HTMP=TURNON-1.                       !swm - LGR added by wschmid
            CALL SGWF2LGR2WETCHK(J,I,K,HTMP,0)   !swm - LGR added by wschmid
            IF (HTMP.GE.TURNON) GO TO 50         !swm - LGR added by wschmid      
         END IF
C
...
C5------ELEVATION HAS BEEN REACHED.
     ...
     1                       AND. HTMP.GE.TURNON) GO TO 50
            ...
            END IF
            IF (ILGR.NE.0) THEN                    !swm - LGR added by wschmid
                HTMP=TURNON-1.                     !swm - LGR added by wschmid
                CALL SGWF2LGR2WETCHK(J,I,K,HTMP,1) !swm - LGR added by wschmid
                IF (HTMP.GE.TURNON) GO TO 50       !swm - LGR added by wschmid
            END IF                                 !swm - LGR added by wschmid            
         END IF
C
