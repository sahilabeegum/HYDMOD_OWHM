      ...
      DOUBLE PRECISION, SAVE, DIMENSION(:), POINTER,CONTIGUOUS :: Alu
      INTEGER, SAVE, DIMENSION(:), POINTER,CONTIGUOUS :: Jlu, Ju
      TYPE GMRESTYPE
        DOUBLE PRECISION, POINTER :: Stop_tol_gmres
        INTEGER, POINTER :: IorderILU, Idir, Msdr
        INTEGER, POINTER :: Ilu_method, Lev_fill
        INTEGER, POINTER :: Maxitr_gmres, Istor_gmres
        DOUBLE PRECISION, POINTER :: DROP_TOL
        DOUBLE PRECISION, DIMENSION(:), POINTER,CONTIGUOUS :: Alu
        INTEGER, DIMENSION(:), POINTER,CONTIGUOUS :: Jlu, Ju
      END TYPE
      TYPE(GMRESTYPE),DIMENSION(10):: GMRESDAT
      CONTAINS
      SUBROUTINE GMRESPNT(IGRID)
      INTEGER::IGRID
       Idir=>GMRESDAT(IGRID)%Idir
       Msdr=>GMRESDAT(IGRID)%Msdr
       Ilu_method=>GMRESDAT(IGRID)%Ilu_method
       Lev_fill=>GMRESDAT(IGRID)%Lev_fill
       Maxitr_gmres=>GMRESDAT(IGRID)%Maxitr_gmres
       Istor_gmres=>GMRESDAT(IGRID)%Istor_gmres
       DROP_TOL=>GMRESDAT(IGRID)%DROP_TOL
       Alu=>GMRESDAT(IGRID)%Alu
       Jlu=>GMRESDAT(IGRID)%Jlu
       Ju=>GMRESDAT(IGRID)%Ju
      END SUBROUTINE
      SUBROUTINE GMRESPSV(IGRID)
       INTEGER::IGRID
       GMRESDAT(IGRID)%Idir=>  Idir
       GMRESDAT(IGRID)%Msdr => Msdr
       GMRESDAT(IGRID)%Ilu_method=>Ilu_method 
       GMRESDAT(IGRID)%Lev_fill=>Lev_fill
       GMRESDAT(IGRID)%Maxitr_gmres=>Maxitr_gmres
       GMRESDAT(IGRID)%Istor_gmres=>Istor_gmres
       GMRESDAT(IGRID)%DROP_TOL=>DROP_TOL
       GMRESDAT(IGRID)%Alu=>Alu
       GMRESDAT(IGRID)%Jlu=>Jlu
       GMRESDAT(IGRID)%Ju=>Ju
      END SUBROUTINE
...
!1------IDENTIFY PACKAGE AND INITIALIZE.
    ...
    1 FORMAT(1X,'GMRES -- LINEAR SOLUTION BY GMRES PACKAGE ',  &
      ...
      /1X,'    using software originally developed by Saad (1990)',  &
      /1X,'    using algorithms described in Saad (2003)')
! ALLOCATE GMRES data
...
!  DEALLOCATE GLOBAL DATA
      ...
      DEALLOCATE(GMRESDAT(IGRID)%Idir        , STAT = ALLOC_ERR)
      DEALLOCATE(GMRESDAT(IGRID)%Msdr        , STAT = ALLOC_ERR)
      DEALLOCATE(GMRESDAT(IGRID)%Ilu_method  , STAT = ALLOC_ERR)
      DEALLOCATE(GMRESDAT(IGRID)%Lev_fill    , STAT = ALLOC_ERR)
      DEALLOCATE(GMRESDAT(IGRID)%Maxitr_gmres, STAT = ALLOC_ERR)
      DEALLOCATE(GMRESDAT(IGRID)%Istor_gmres , STAT = ALLOC_ERR)
      DEALLOCATE(GMRESDAT(IGRID)%DROP_TOL    , STAT = ALLOC_ERR)
      DEALLOCATE(GMRESDAT(IGRID)%Alu         , STAT = ALLOC_ERR)
      DEALLOCATE(GMRESDAT(IGRID)%Jlu         , STAT = ALLOC_ERR)
      DEALLOCATE(GMRESDAT(IGRID)%Ju          , STAT = ALLOC_ERR)
!
