      MODULE OBSRIVMODULE
         ...
         INTEGER, SAVE, DIMENSION(:),   POINTER,CONTIGUOUS::NQOBRV
         INTEGER, SAVE, DIMENSION(:),   POINTER,CONTIGUOUS::NQCLRV
      ...
      TYPE OBSRIVTYPE
         ...
         INTEGER,     DIMENSION(:),   POINTER,CONTIGUOUS::NQOBRV
         INTEGER,     DIMENSION(:),   POINTER,CONTIGUOUS::NQCLRV
    ...
    9 FORMAT (/,
     ...
     &     '   NUMBER OF RIVER-CELL FLOWS.....................: ',I6)
      IF(NQTRV.LE.0) THEN
         IF(LSTCHK(1)) THEN
           WRITE(IOUT,*) 
     1        ' NUMBER OF OBSERVATIONS LESS THAN OR EQUAL TO 0'
         ENDIF
         CALL USTOP(' ')
      END IF
...
C  Deallocate OBSRIV memory
      USE OBSRIVMODULE
C
      DEALLOCATE(OBSRIVDAT(IGRID)%NQRV)
      DEALLOCATE(OBSRIVDAT(IGRID)%NQCRV)
      DEALLOCATE(OBSRIVDAT(IGRID)%NQTRV)
      DEALLOCATE(OBSRIVDAT(IGRID)%IURVOBSV)
      DEALLOCATE(OBSRIVDAT(IGRID)%IPRT)
      DEALLOCATE(OBSRIVDAT(IGRID)%NQOBRV)
      DEALLOCATE(OBSRIVDAT(IGRID)%NQCLRV)
      DEALLOCATE(OBSRIVDAT(IGRID)%IOBTS)
      DEALLOCATE(OBSRIVDAT(IGRID)%FLWSIM)
      DEALLOCATE(OBSRIVDAT(IGRID)%FLWOBS)
      DEALLOCATE(OBSRIVDAT(IGRID)%TOFF)
      DEALLOCATE(OBSRIVDAT(IGRID)%OTIME)
      DEALLOCATE(OBSRIVDAT(IGRID)%QCELL)
      DEALLOCATE(OBSRIVDAT(IGRID)%OBSNAM)
C
