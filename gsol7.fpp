...
C-------CONJUGATE GRADIENT ITERATIVE SOLVER
     ...
     6                     D,P,Q,Z,T)
Cseb ADD CHECK IF IEEE_ARITHMATIC IS availible. If availible for other compilers than include
...
C           UPDATE Q WITH A AND P
          ...
          IF ( denom.EQ.DZERO .OR. IEEE_IS_NAN(denom) )  THEN           !seb added check for if IEE MODULE AVAILBLE
#elif  defined( __GFORTRAN__ ) 
          IF ( denom.EQ.DZERO .OR. isnan(denom) )  THEN
#elif  defined( __G95__ ) 
        IF ( denom.EQ.DZERO .OR. isnan(denom) )  THEN
#else 
        IF ( denom.EQ.DZERO ) THEN
#endif
            ICNVG = -1
            EXIT INNER
          END IF
