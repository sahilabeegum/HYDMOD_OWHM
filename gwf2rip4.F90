...
!
      Read(IN,'(A)') LINE
      If (IFREFM == 0) Then
        Read(LINE,'(4I10)') MAXRIP,MAXPOLY,IRIPCB,IRIPCB1
        LLOC=41
      Else
        LLOC=1
...
!
      Do NC=1, NRIPCL
        If(IFREFM == 0) then
          Read(IN,'(4I10)') (CLOC(NC,I),I=1,3),NPOLY(NC)
        Else
...
!     *************************************************************************
      ...
      USE GWFRIPMODULE, Only:MAXRIP,MAXPOLY,MAXTS,NRIPCL,IRIPCB,IRIPCB1, &
                            C1,C2,CLoc,NPoly,HSurf,CovPFSG,RIPNM,RIPET
...
!
      IF(IBD==2) THEN         
        CALL UBDSV4(KSTP,KPER,TEXT,NAUX,RIPAUX,IRIPCB,NCOL,NROW,NLAY,  &
              NRIPCL,IOUT,DELT,PERTIM,TOTIM,IBOUND)
      END IF
!5----Initialize BUFF
        BUFF = zero
!
...
!
      If(IRIPCB1>0) Then
        ...
        Write(IRIPCB1,'(100I3)') (NPOLY(I),I=1,NRIPCL)        
        DO LC=1,NRIPCL
          IL=CLoc(LC,1)
          IR=CLoc(LC,2)
          IC=CLoc(LC,3)
