...
! Cell property data
        ...
        INTEGER, SAVE,    DIMENSION(:,:,:), POINTER,CONTIGUOUS::IBOUND2
...
!         IF(LAYTYPUPW(K).EQ.0) THEN
     ...
     1          ' LAYWET is not 0 and wetting does not apply in UPW '
              WRITE(IOUT,*)
     1          ' LAYWET must be 0 when using the UPW Package'
            ENDIF
            CALL USTOP(' ')
!         ELSE
!            NWETD=NWETD+1
...
!      END IF
! ALLOCATE THESE BECAUSE THEY ARE LEFT IN THE CODE (DEALLOCATED)  ADDED BY RGN
...
C seb added output of final array
      IF(PROPPRINT.NE.0) CALL PRINTARRAY(IGRID,2)
...
C
      ONE=1.0D0
      IF(DELT.EQ.ZERO)THEN                                              !seb ADDED CHECK FOR WHEN DELT=0, THIS WILL SET ALL STORAGES TO ZERO
        TLED=ZERO
      ELSE
        TLED=ONE/DBLE(DELT)
      END IF
