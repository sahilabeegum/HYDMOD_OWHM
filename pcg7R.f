      MODULE PCGMODULE
        ...
        DOUBLE PRECISION,SAVE, POINTER,DIMENSION(:,:,:),CONTIGUOUS::VPCG
        DOUBLE PRECISION,SAVE, POINTER,DIMENSION(:,:,:),CONTIGUOUS::SS
        DOUBLE PRECISION,SAVE, POINTER,DIMENSION(:,:,:),CONTIGUOUS::P
        DOUBLE PRECISION,SAVE, POINTER,DIMENSION(:,:,:),CONTIGUOUS::HPCG
        REAL,            SAVE, POINTER,DIMENSION(:,:,:),CONTIGUOUS::CD
        REAL,            SAVE, POINTER,DIMENSION(:,:,:),CONTIGUOUS::HCSV
        INTEGER,         SAVE, POINTER,DIMENSION(:,:)  ,CONTIGUOUS::LHCH
        REAL,            SAVE, POINTER,DIMENSION(:)    ,CONTIGUOUS::HCHG
        INTEGER,        SAVE, POINTER,DIMENSION(:,:),CONTIGUOUS::LRCHPCG
        REAL,            SAVE, POINTER,DIMENSION(:)    ,CONTIGUOUS::RCHG
        INTEGER,         SAVE, POINTER,DIMENSION(:)    ,CONTIGUOUS::IT1
      TYPE PCGTYPE
        ...
        DOUBLE PRECISION,  POINTER, DIMENSION(:,:,:),CONTIGUOUS::VPCG
        DOUBLE PRECISION,  POINTER, DIMENSION(:,:,:),CONTIGUOUS::SS
        DOUBLE PRECISION,  POINTER, DIMENSION(:,:,:),CONTIGUOUS::P
        DOUBLE PRECISION,  POINTER, DIMENSION(:,:,:),CONTIGUOUS::HPCG
        REAL,              POINTER, DIMENSION(:,:,:),CONTIGUOUS::CD
        REAL,              POINTER, DIMENSION(:,:,:),CONTIGUOUS::HCSV
        INTEGER,           POINTER, DIMENSION(:,:)  ,CONTIGUOUS::LHCH
        REAL,              POINTER, DIMENSION(:)    ,CONTIGUOUS::HCHG
        INTEGER,           POINTER, DIMENSION(:,:)  ,CONTIGUOUS::LRCHPCG
        REAL,              POINTER, DIMENSION(:)    ,CONTIGUOUS::RCHG
        INTEGER,           POINTER, DIMENSION(:)    ,CONTIGUOUS::IT1
      END TYPE
...
C-------CHANGE IN HEAD AND THE LARGEST VALUE OF THE RESIDUAL.
      KH=0                                                              !seb ADDED INTIALIZATION TO PREVENT ERROR WHEN A LARGEST VALUE IS NOT FOUND (OCCURS WHEN PERFECT ANSWER IS MET)
      IH=0
      JH=0
      KR=0
      IR=0
...
C     ------------------------------------------------------------------
      ...
      REAL C, RES
      REAL(DEFAULT_KIND):: CC, CR, CV                                   !seb ORIGINALLY DEFINED AS REAL
      DOUBLE PRECISION HCOF                                             !seb ORIGINALLY DEFINED AS REAL
...
C  Deallocate PCG DATA
      USE PCGMODULE
C
        DEALLOCATE(PCGDAT(IGRID)%ITER1    )
        DEALLOCATE(PCGDAT(IGRID)%NPCOND   )
        DEALLOCATE(PCGDAT(IGRID)%NBPOL    )
        DEALLOCATE(PCGDAT(IGRID)%IPRPCG   )
        DEALLOCATE(PCGDAT(IGRID)%MUTPCG   )
        DEALLOCATE(PCGDAT(IGRID)%NITER    )
        DEALLOCATE(PCGDAT(IGRID)%HCLOSEPCG)
        DEALLOCATE(PCGDAT(IGRID)%RCLOSEPCG)
        DEALLOCATE(PCGDAT(IGRID)%RELAXPCG )
        DEALLOCATE(PCGDAT(IGRID)%DAMPPCG  )
        DEALLOCATE(PCGDAT(IGRID)%DAMPPCGT )
        DEALLOCATE(PCGDAT(IGRID)%IHCOFADD )
        DEALLOCATE(PCGDAT(IGRID)%VPCG     )
        DEALLOCATE(PCGDAT(IGRID)%SS       )
        DEALLOCATE(PCGDAT(IGRID)%P        )
        DEALLOCATE(PCGDAT(IGRID)%HPCG     )
        DEALLOCATE(PCGDAT(IGRID)%CD       )
        DEALLOCATE(PCGDAT(IGRID)%HCSV     )
        DEALLOCATE(PCGDAT(IGRID)%LHCH     )
        DEALLOCATE(PCGDAT(IGRID)%HCHG     )
        DEALLOCATE(PCGDAT(IGRID)%LRCHPCG  )
        DEALLOCATE(PCGDAT(IGRID)%RCHG     )
        DEALLOCATE(PCGDAT(IGRID)%IT1      )
C
