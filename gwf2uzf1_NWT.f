...
C SWR - JDH ADDED Iunitswr
     ...
     +                      Iunitcfp, Iunitnwt, Iunitswr, Igrid)
C     ******************************************************************
...
!               REJ_INF(ic, ir) = cellarea * finfhold
            ...
            END IF
          END IF
!          REJ_INF(ic, ir) = cellarea * (finfhold-finfact)  !RICH COMMENTED THIS OUT
C
...
C37-----SAVE INFILTRATION RATES TO UNFORMATTED FILE.
     ...
     +                                   PERTIM, TOTIM, IBOUND)
      ...
      END IF
C
C35-----UPDATE RATES AND BUFFERS FOR SFR-DIVERTED INFILTRATION. COMMENTED OUT BY RICH
...
C
C9------CONVERT TRAIL WAVES TO LEAD TRAIL WAVES. Modified to prevent taking exponent of a negative number rth/seb
          ELSE IF ( itrwaveb.GT.1 ) THEN
            DO k = jpntp1, Jpnt + itrwaveb
              Itrwave(k) = 0
              Ltrail(k) = 1
              IF ( ABS(Theta(k)-Theta(k-1)).LT.CLOSEZERO ) THEN
                if(Theta(k).Gt.Thetar)then
                  fhold = ((Theta(k)-Thetar)*thsrinv)**Epsd
                  IF( fhold.LT.NEARZERO ) fhold = 0.0D0
                else
                  fhold=0.0D0
                end if                      
              ...
              ELSE
                if(Theta(k-1).Gt.Thetar)then
                 fhold = ((Theta(k-1)-Thetar)*thsrinv)**Epsd
                 IF( fhold.LT.NEARZERO ) fhold = 0.0D0
                else
                 fhold=0.0D0
                end if
...
C         WHEN LEAD TRAIL WAVE INTERSECTS A LEAD WAVE.  Modified to prevent taking exponent of a negative number rth/seb              
