...
Crgn    Added budget variables for GSFLOW CSV file
        ...
        INTEGER,SAVE, DIMENSION(:),  POINTER,CONTIGUOUS ::ICS, NCNCVR,  
     +                                         LIMERR,LAKTAB
        INTEGER,SAVE, DIMENSION(:,:),POINTER,CONTIGUOUS ::ILAKE,ITRB,
     +                                                     IDIV,ISUB,IRK
        INTEGER,SAVE, DIMENSION(:,:,:),POINTER,CONTIGUOUS ::LKARR1
        REAL,   SAVE, DIMENSION(:),  POINTER,CONTIGUOUS ::STAGES
        DOUBLE PRECISION,SAVE,DIMENSION(:), POINTER,CONTIGUOUS::STGNEW,
     +                                    STGOLD,STGITER,VOLOLDD,STGOLD2
        REAL,   SAVE, DIMENSION(:), POINTER,CONTIGUOUS ::VOL,FLOB,DSRFOT
        DOUBLE PRECISION,SAVE,DIMENSION(:),POINTER,CONTIGUOUS ::PRCPLK,
     +                                                          EVAPLK
        ...
        REAL,SAVE,DIMENSION(:),POINTER,CONTIGUOUS::BOTTMS,BGAREA,SSMN,
     +                                             SSMX
Cdep    Added cumulative and time step error budget arrays
        REAL,SAVE, DIMENSION(:),POINTER,CONTIGUOUS::CUMVOL,CMLAKERR,
     +                                             CUMLKOUT
        REAL,SAVE, DIMENSION(:),POINTER,CONTIGUOUS::CUMLKIN,TSLAKERR,
     +                                              DELVOL
crgn        REAL,   SAVE, DIMENSION(:),  POINTER ::EVAP,PRECIP,SEEP,SEEP3
        DOUBLE PRECISION,SAVE,DIMENSION(:),POINTER,CONTIGUOUS::EVAP,
     +                                                         PRECIP
        DOUBLE PRECISION,SAVE,DIMENSION(:),POINTER,CONTIGUOUS::EVAP3,
     +                                                         PRECIP3
        DOUBLE PRECISION,SAVE,DIMENSION(:),POINTER,CONTIGUOUS::FLWITER
        DOUBLE PRECISION,SAVE,DIMENSION(:),POINTER,CONTIGUOUS::FLWITER3
        DOUBLE PRECISION,SAVE,DIMENSION(:),POINTER,CONTIGUOUS::SEEP,
     +                                                         SEEP3
...
C         when Solute Transport is active
        REAL,   SAVE, DIMENSION(:,:),POINTER,CONTIGUOUS ::XLAKES,
     +                                                   XLAKINIT,XLKOLD
Crsr    Allocate arrays in BD subroutine
        INTEGER,SAVE, DIMENSION(:),  POINTER,CONTIGUOUS::LDRY,NCNT,
     +                                                   NCNST,KSUB
        INTEGER,SAVE, DIMENSION(:),  POINTER,CONTIGUOUS::MSUB1
        INTEGER,SAVE, DIMENSION(:,:),POINTER,CONTIGUOUS::MSUB
        REAL,   SAVE, DIMENSION(:),  POINTER,CONTIGUOUS::FLXINL,VOLOLD,
     +                                                   GWIN,GWOUT
        REAL,   SAVE, DIMENSION(:),  POINTER,CONTIGUOUS::DELH,TDELH,
     +                                                   SVT,STGADJ
...
Crgn    Added budget variables for GSFLOW CSV file
        ...
        INTEGER,      DIMENSION(:),  POINTER,CONTIGUOUS ::ICS, NCNCVR, 
     +                                                    LIMERR, LAKTAB
        INTEGER,      DIMENSION(:,:),POINTER,CONTIGUOUS ::ILAKE,ITRB,
     +                                                    IDIV,ISUB,IRK
        INTEGER,      DIMENSION(:,:,:),POINTER,CONTIGUOUS ::LKARR1
        REAL,         DIMENSION(:),  POINTER,CONTIGUOUS ::STAGES
        DOUBLE PRECISION,DIMENSION(:),POINTER,CONTIGUOUS ::STGNEW,
     +                                        STGOLD,STGITER,STGOLD2
...
Crsr    Allocate arrays in BD subroutine
        ...
        REAL,   DIMENSION(:),  POINTER,CONTIGUOUS::FLXINL,VOLOLD,GWIN,
     +                                                           GWOUT
...
C
  ...
  802 FORMAT(1H0,'REUSING RECH,ET,WITHDRAWAL RATES FROM LAST STRESS PERI
     1OD'/)
      GOTO 900
  801 IF(LSTCHK(3)) THEN
        IF(ISS.NE.0.AND.KKPER.GT.1) WRITE(IOUT,7)
      ENDIF
7     FORMAT(/1X,'LAKE',7X,'PRECIP',5X,'EVAP',5X,'RUNOFF',
 ...
 8    FORMAT (//1X,'LAKE',4X,'SOLUTE',6X,'CPPT',6X,'CRNF',6X,'CAUG'/)
     ...
     1   RNF(LM),WTHDRW(LM)
      END IF
      IF(LSTCHK(3)) THEN
        IF(ISS.NE.0.AND.KKPER.GT.1) WRITE(IOUT,9) LM,PRCPLK(LM),
     1   EVAPLK(LM),RNF(LM),WTHDRW(LM),BOTTMS(LM),BGAREA(LM),SSMN(LM),
     1   SSMX(LM)
      ENDIF
9     FORMAT(1X,I3,4X,1P,3E10.3,1X,5E10.3)
      IF(LSTCHK(3)) THEN
        IF(ISS.EQ.0.OR.KKPER.EQ.1) WRITE(IOUT,9) LM,PRCPLK(LM),
     1   EVAPLK(LM),RNF(LM),WTHDRW(LM),BOTTMS(LM),BGAREA(LM)
      ENDIF
...
C7------RETURN
      ...
      END
C
      SUBROUTINE GWF2LAK7AD(KKPER,KKSTP,IUNITBCF,IUNITLPF,IUNITHUF,
     +                      IUNITSFR,IUNITUZF,IUNITUPW,IUNITGWT,IGRID)
C
C------OLD USGS VERSION 7.1; JUNE 2006 GWF2LAK7AD; REVISED FEBRUARY 6, 2012
C------REVISION NUMBER CHANGED TO BE CONSISTENT WITH NWT RELEASE
C------NEW VERSION NUMBER 1.0.5:  April 5, 2012  
C------NEW VERSION NUMBER 1.0.6:  December 31, 2012  R.T. Hanson Added SUBLINK capability to Lake Package
C------NEW VERSION NUMBER 1.0.9:  July 1, 2014  
C
C     ******************************************************************
...
C     SPECIFICATIONS:
C     ------------------------------------------------------------------
      USE GWFLAKMODULE, ONLY: NLAKES, LKNODE, FLOB, STAGES,ILAKE,
     +                        STGNEW, STGOLD, VOLOLDD, VOLOLD, VOLINIT,
     +                        BOTTMS, IDIV, STGOLD2, NDV
      USE GWFSFRMODULE, ONLY: DLKSTAGE
      USE GLOBAL,       ONLY: IOUT,IBOUND,BOTM,LBOTM
...
Cdep  initialized VOLINIT and VOLOLD to VOLOLDD 6/4/2009
      DO I=1,NLAKES
        ...
        ELSE
          ...
          VOLOLD(I)=VOLOLDD(I)
        END IF 
ccrth Update BOTTMS if LPFLNK is activated from Subsidence linkage
        IF(ASSOCIATED(LPFLNK))THEN                                      !seb ADDED CHECK INCASE LPFLNK IS NOT ALLOCATED
        if(LPFLNK)then
         DO IDV=1,NDV
           INODE=IDIV(I,IDV)
          IF (INODE.GT.0) THEN
           ...
           IF(IBOUND(IC,IR,IL).GT.0) THEN
            BOTTMS(I) = BOTTMS(I) - DVZ(IC,IR,IL)
           IF (IUNITBCF.GT.0) THEN  ! rth, update vertical and horizontal conductances based on changes in BOTM with SUBLNK
            CALL SGWF2LAK7BCF7RPS()
           ELSE IF (IUNITLPF.GT.0) THEN
            CALL SGWF2LAK7LPF7RPS()
           ELSE IF (IUNITHUF.GT.0) THEN
...
C16E----LINEAR CASE. SIMPLY CALCULATE STAGE BASED ON VOLUME.
                  ...
                  NCNCVR(LAKE) = 1
                END IF
!      IF (lake==1)then                                                    !seb commented out output to lst file
...
!        write(iout,222)PRECIP3(LAKE),EVAP3(LAKE),RUNF,RUNOFF,             !This was commented out before seb
!     1                WITHDRW3,SURFIN(LAKE),OUTFLOW,
...
C-LFK
      ...
      ELSE
       IF(LSTCHK(3)) THEN
         WRITE(IOUT,2055) NN,CUMWDR(NN),CUMFLX(NN),CUMVOL(NN),
     1    CMLAKERR(NN)
       ENDIF
      END IF
 2105 CONTINUE
