...
C--USE FILE SPECIFICATION of MODFLOW-2005
     ...
     &            MTIBS,MTLAK,MTMNW,MTSWT,MTSFR,MTUZF,MTFMP,MTRIP,MTSWI,
     &            MTSWR,MTSFR,MTSUB /28*0/
ccrth      INTEGER  MTMNW1,MTMNW2
C     -----------------------------------------------------------------    
...
!swm: store separate to not get clobbered by MNW1
        ...
        ELSEIF(CUNIT(IU).EQ.'SWR ') THEN   !rth
          MTSWI=IUNIT(IU)
        ELSEIF(CUNIT(IU).EQ.'SFR ') THEN   !rth
          MTSWI=IUNIT(IU)
        ELSEIF(CUNIT(IU).EQ.'SUB ') THEN   !rth
          MTSWI=IUNIT(IU)
        ENDIF
      ENDDO 
...
C--WRITE A HEADER TO MODFLOW-MT3DMS LINK FILE
     ...
     &     MTSWT,MTSFR,MTUZF,MTFMP,MTRIP,MTSWI,MTSWR,MTSFR,MTSUB
        ELSEIF(ILMTFMT.EQ.1) THEN
          WRITE(IUMT3D,*) 'MT3D4.00.00',
     &     MTWEL,MTDRN,MTRCH,MTEVT,MTRIV,MTGHB,MTCHD,MTISS,MTNPER,
     &     MTSTR,MTRES,MTFHB,MTDRT,MTETS,MTSUB,MTIBS,MTLAK,MTMNW,
...
ccrth--added additional features added to OWHM version of MODFLOW FMP,RIP,SWI,SWR,SFR,SUB
!        IF(IUNIT(61).GT.0) 
!     &   CALL LMT7FMP7(ILMTFMT,IUMT3D,KKSTP,KKPER,IGRID)
!        IF(IUNIT(6).GT.0) 
!     &   CALL LMT7RIP7(ILMTFMT,IUMT3D,KKSTP,KKPER,IGRID)
!        IF(IUNIT(65).GT.0) 
!     &   CALL LMT7SWI7(ILMTFMT,IUMT3D,KKSTP,KKPER,IGRID)
!        IF(IUNIT(64).GT.0) 
!     &   CALL LMT7SWR7(ILMTFMT,IUMT3D,KKSTP,KKPER,IGRID)
!        IF(IUNIT(44).GT.0) 
!     &   CALL LMT7SFR7(ILMTFMT,IUMT3D,KKSTP,KKPER,IGRID)
!        IF(IUNIT(54).GT.0) 
!     &   CALL LMT7SUB7(ILMTFMT,IUMT3D,KKSTP,KKPER,IGRID)
ccrth
C
      RETURN
      END
...
C--FOR EACH CELL CALCULATE FLOW THRU LOWER FACE & STORE IN BUFFER
 ...
 !               IF(TMP.LT.TOP) HD=TOP
 !             ENDIF
!              IF ( HNEW(J,I,K).GT.BOTM(J,I,K) ) THEN      !RGN COMMENTED THIS
                HDIFF=HNEW(J,I,K)-HD
                BUFF(J,I,K)=HDIFF*CV(J,I,K)
!              END IF
            ENDIF
...
C--FOR EACH CELL CALCULATE GW ET & STORE IN BUFFER
      DO I=1,NROW
        DO J=1,NCOL
          K = ABS(IUZFBND(J,I))  !Rich ADDED ABS
          IF ( K.NE.0 ) THEN
            IF ( IBOUND(J,I,K).GT.0 ) THEN
              BUFF(J,I,K)=-GWET(J,I)
            END IF
...
C--RECORD CONTENTS OF BUFFER.
      ...
      END
C***********************************************************************
!      SUBROUTINE LMT7FMP7(ILMTFMT,IUMT3D,KSTP,KPER,IGRID)
!C *********************************************************************
!C SAVE FLOW THROUGH SOIL ZONE IN VERTICAL DIRECTION.
!C THIS SUBROUTINE IS CALLED ONLY IF THE 'FMP' PROCESS
!C IS USED IN MODFLOW AND SOLUTE ROUTING IS ACTIVE IN 'FMP'.
!C *********************************************************************
!C
!      USE GLOBAL,      ONLY:NCOL,NROW,NLAY,IBOUND,HNEW,HOLD,
!     &                      BUFF,BOTM,DELR,DELC
!      USE GWFBASMODULE,ONLY:DELT, HNOFLO
!!      USE GWFFMPMODULE,ONLY:IFID, GRIDET, IUZHOLD, IFNRFLG, 
!!     &                      SGWET,EEPOUT, RTSOLUTE, IUZFOPT
!      CHARACTER*16 TEXT1, TEXT2
!      REAL cellarea
!ccrth Subroutine disabled until ET I/O finished
!      RETURN
!ccrth
!!      IF ( RTSOLUTE.LE.0 ) RETURN
!!      IF ( IETFLG.LE.0 ) RETURN
...
!      CALL SFMP3PNT(IGRID)
!C            
!      TEXT1='FMP-ET' !outflow part of Farm Net Recharge
!
!C
!C--CLEAR THE BUFFER FOR FMP ET
!      DO IL=1,NLAY
!        DO IR=1,NROW
!          DO IC=1,NCOL
!            BUFF(IC,IR,IL)=0.0
...
!      numcells = NCOL*NROW
!C
!C--FOR EACH CELL CALCULATE FMP ET & STORE IN BUFFER
!ccrth Calculation block goes here
!C
!C
!C--RECORD CONTENTS OF BUFFER.
!      IF(ILMTFMT.EQ.0) THEN
...
!      END
C***********************************************************************
!      SUBROUTINE LMT7RIP7(ILMTFMT,IUMT3D,KSTP,KPER,IGRID)
!C *********************************************************************
!C SAVE FLOW THROUGH SOIL ZONE IN VERTICAL DIRECTION.
!C THIS SUBROUTINE IS CALLED ONLY IF THE 'RIP' PACKAGE
!C IS USED IN MODFLOW AND SOLUTE ROUTING IS ACTIVE IN 'RIP'.
!C *********************************************************************
!C
!      USE GLOBAL,      ONLY:NCOL,NROW,NLAY,IBOUND,HNEW,HOLD,
!     &                      BUFF,BOTM,DELR,DELC
!      USE GWFBASMODULE,ONLY:DELT, HNOFLO
!!      USE GWFRIPMODULE,ONLY:IFID, GRIDET, IUZHOLD, IFNRFLG, GWET, !needs to be updated before used
!!     &                      SEEPOUT, RTSOLUTE, IUZFOPT
!      CHARACTER*16 TEXT1, TEXT2
!      REAL cellarea
!ccrth Subroutine disabled until ET I/O finished
...
!      END
C***********************************************************************
!      SUBROUTINE LMT7SWI7(ILMTFMT,IUMT3D,KSTP,KPER,IGRID)
!C *********************************************************************
!C SAVE FLOW THROUGH SALTWATER INTERFACE IN LATERAL DIRECTION.
!C THIS SUBROUTINE IS CALLED ONLY IF THE 'SWI' PACKAGE
!C IS USED IN MODFLOW AND SOLUTE ROUTING IS ACTIVE IN 'SWI'.
!C *********************************************************************
!C
!      USE GLOBAL,      ONLY:NCOL,NROW,NLAY,IBOUND,HNEW,HOLD,
!     &                      BUFF,BOTM,DELR,DELC
!      USE GWFBASMODULE,ONLY:DELT, HNOFLO
!!      USE GWFSWIMODULE,ONLY:IFID, GRIDET, IUZHOLD, IFNRFLG, GWET, !needs to be updated before used
!!     &                      SEEPOUT, RTSOLUTE, IUZFOPT
!      CHARACTER*16 TEXT1, TEXT2
!      REAL cellarea
!ccrth Subroutine disabled until flows I/O finished
!      RETURN
!ccrth
!!      IF ( RTSOLUTE.LE.0 ) RETURN
!!      IF ( IETFLG.LE.0 ) RETURN
...
!C *********************************************************************
!C SAVE FLOW THROUGH channel Bottoms IN Vertical DIRECTION.
!C THIS SUBROUTINE IS CALLED ONLY IF THE 'SWR' PROCESS
!C IS USED IN MODFLOW AND SOLUTE ROUTING IS ACTIVE IN 'SWR'.
!C *********************************************************************
!C
!      USE GLOBAL,      ONLY:NCOL,NROW,NLAY,IBOUND,HNEW,HOLD,
!     &                      BUFF,BOTM,DELR,DELC
!      USE GWFBASMODULE,ONLY:DELT, HNOFLO
!!      USE GWFSWRMODULE,ONLY:IFID, GRIDET, IUZHOLD, IFNRFLG, GWET, !needs to be updated before used
!!     &                      SEEPOUT, RTSOLUTE, IUZFOPT
!      CHARACTER*16 TEXT1, TEXT2
!      REAL cellarea
!ccrth Subroutine disabled until flows I/O finished
...
!      END
C***********************************************************************
!      SUBROUTINE LMT7SFR7(ILMTFMT,IUMT3D,KSTP,KPER,IGRID)
!C *********************************************************************
!C SAVE FLOW THROUGH channel Bottoms IN Vertical DIRECTION.
!C THIS SUBROUTINE IS CALLED ONLY IF THE 'SFR' PACKAGE
!C IS USED IN MODFLOW AND SOLUTE ROUTING IS ACTIVE IN 'SFR'.
!C *********************************************************************
!C
!      USE GLOBAL,      ONLY:NCOL,NROW,NLAY,IBOUND,HNEW,HOLD,
!     &                      BUFF,BOTM,DELR,DELC
!      USE GWFBASMODULE,ONLY:DELT, HNOFLO
!!      USE GWFSFRMODULE,ONLY:IFID, GRIDET, IUZHOLD, IFNRFLG, GWET, !needs to be updated before used
!!     &                      SEEPOUT, RTSOLUTE, IUZFOPT
!      CHARACTER*16 TEXT1, TEXT2
!      REAL cellarea
!ccrth Subroutine disabled until flows I/O finished
...
!      END
C***********************************************************************
!      SUBROUTINE LMT7SUB7(ILMTFMT,IUMT3D,KSTP,KPER,IGRID)
!C *********************************************************************
!C SAVE FLOW THROUGH Interbeds/Confining Units IN Vertical DIRECTION.
!C THIS SUBROUTINE IS CALLED ONLY IF THE 'SUB' PACKAGE
!C IS USED IN MODFLOW AND SOLUTE ROUTING IS ACTIVE IN 'SUB'.
!C *********************************************************************
!C
!      USE GLOBAL,      ONLY:NCOL,NROW,NLAY,IBOUND,HNEW,HOLD,
!     &                      BUFF,BOTM,DELR,DELC
!      USE GWFBASMODULE,ONLY:DELT, HNOFLO
!!      USE GWFSUBMODULE,ONLY:IFID, GRIDET, IUZHOLD, IFNRFLG, GWET, !needs to be updated before used
!!     &                      SEEPOUT, RTSOLUTE, IUZFOPT
!      CHARACTER*16 TEXT1, TEXT2
!      REAL cellarea
!ccrth Subroutine disabled until flows I/O finished
