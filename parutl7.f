...
C5------Too many parameters.
      IF(LSTCHK(1)) THEN
        WRITE(IOUT,99)
        WRITE(IOUT,'(A,3(/,A,I10,A,I10),/,A,/,A)')
     +  'THE CURRENT STORAGE FOR PARAMETERS IS: ',
     +  '# PARAMETER=',IPSUM,    ' WITH MAX STORAGE (MXPAR)  OF',MXPAR,
     +  '# INSTANCES=',INAMLOC-1,' WITH MAX STORAGE (MXINST) OF',MXINST,
     +  '# CLUSTERS =',ICLSUM,   ' WITH MAX STORAGE (MXCLST) OF',MXCLST,
