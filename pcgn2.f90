MODULE PCGN
  ...
  TYPE PCGNTYPE
     ...
     DOUBLE PRECISION, DIMENSION(:), CONTIGUOUS, POINTER :: DD, DX, DY, DZ, RES, HCH          !seb added CONTIGUOUS FLAG. IF THIS CAUSES PROBLEMS, REMOVE IT
  END TYPE PCGNTYPE
  ...
  ! ... POINTERS
  INTEGER, DIMENSION(:), CONTIGUOUS, POINTER :: IBOUND          !seb added CONTIGUOUS FLAG. IF THIS CAUSES PROBLEMS, REMOVE IT
  DOUBLE PRECISION, DIMENSION(:), CONTIGUOUS, POINTER :: CR, CC, CV, HCOF, RHS
  DOUBLE PRECISION, DIMENSION(:), CONTIGUOUS, POINTER :: HNEW
...
415       FORMAT (I10,F10.0,I10,F10.0,I10)
    ...
    ELSE
       ...
       IF (IOS/=0) THEN
          ...
          STOP
       ENDIF
       IF(IFILL.NE.1 .AND. IFILL.NE.0)THEN
          IF(LSTCHK(1)) THEN
            WRITE (IOUT,'(A)') 'PCGN ERROR IFILL NOT EQUAL TO 0 OR 1'
          ENDIF
          STOP 'PCGN ERROR IFILL NOT EQUAL TO 0 OR 1'
       END IF
       IF (ITER_MO>1) THEN
          ! ... NONLINEAR DAMPING
...
810 FORMAT (1X,/,1X,20X,'***WARNING!!!***',/, &
  ...
  ! ... *****************************************************************
       SUBROUTINE PCGN2AP(HNEWAP,RHSAP,CRAP,CCAP,CVAP,HCOFAP,IBDAP,KKITER,&
                  KKSTP,KKPER,ICNVG,HNOFLO,IGRID)
    ! ... 
    ! ... ***************************************************************
    ...
    DOUBLE PRECISION, DIMENSION(*), TARGET, INTENT(IN) :: CRAP, CCAP, CVAP, HCOFAP
    DOUBLE PRECISION, DIMENSION(*), TARGET, INTENT(INOUT)  :: RHSAP
    ...
    DO K=1,NLAY
       ...
       DO I=1,NROW
          ...
          DO J=1,NCOL
             ...
             EE=EE - HCOF(N) !seb already DP in OWHM REMOVED REAL(,8)
             DD(N)=EE
             RR=RR-EE*HNEW(N)
             RES(N)=RR
             RTR=RTR+RR**2
