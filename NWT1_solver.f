...
!      RICHARD G. NISWONGER
     ...
     2                     LBOTM,HNEW, IUNIT
      USE GLOBAL,     ONLY:LSTCHK,LENUNI                                !seb added additional global var
     ...
     +      MASK=IBOUND(:,:,:).NE.0 )      
      ...
      DO IC = 1, NCOL
       IF ( IBOUND(IC,IR,IL).GT.0 ) THEN
        ...
        IF (THK .LT. THICKFACT * MXTHCK ) THEN                          !THIS MAY NEED TO BE MULTIPLIED BY 10 TO MAKE MORE SENSITIVE
           IF(LSTCHK(1).AND.I.EQ.0) THEN                                !WRITE OUT WARNING HEADER
               WRITE(IOUT,'(/ A )') REPEAT('#',35)//
     +    '     NWT/UPW  WARNING    ' // REPEAT('#',80)//NEW_LINE(' ')//
     +                 REPEAT('#',140)// NEW_LINE(' ') //REPEAT('#',140)
               I=1
           END IF
           IF(LSTCHK(1)) WRITE(IOUT,'(/ A,G12.5,A,3I5,A,G12.5,A,/A)') 
     +       'WARNING: EXTREMELY THIN CELL WITH THICKNESS OF ',
     +       THK,', FOR LAYER, ROW, COL: ',IL,IR,IC, 
     +     ' WITH MAX TOLERANCE OF, ', THICKFACT * MXTHCK,
...
C seb Build HFB ORDERCELL INDEX
      IF(IUNIT(21).GT.0) CALL ORDERCELLHFB()
      CALL COUNTACTIVE(jj)
...
!          IF ( IL.LT.NLAY ) isum = isum + abs(Ibound(ic, ir, il+1)) !RICH REMOVED THIS
...
!             matrix=22 !commented out by rgn
...
!             control=1  !commented out by rgn
...
C#######################################################################
            ...
            ELSEIF ( ill.GT.il.AND.irr.EQ.ir.AND.icc.EQ.ic ) THEN
              ...
              sum = sum + A(I)
            ELSEIF ( irr.LT.ir ) THEN
              A(I) = (Ccm1 + Dc(JA(I),5)*(Hrm1-H))                      !seb CHANGED Dc(Icell(ic,ir-1,il),5) to Dc(JA(I),5)
              BB(ij) = BB(ij) + A(I)*Hrm1
              sum = sum + A(I)
            ELSEIF ( icc.LT.ic ) THEN
C seb CHANGED Dc(Icell(ic,ir-1,il),4) to Dc(JA(I),4)
              A(I) = (Crm1 + Dc(JA(I),4)*(Hcm1-H))                      !seb CHANGED Dc(Icell(ic,ir-1,il),4) to Dc(JA(I),4)
              BB(ij) = BB(ij) + A(I)*Hcm1
              sum = sum + A(I)
            ELSEIF ( icc.GT.ic ) THEN
              A(I) = (Crr  + Dc(JA(I),3)*(Hcp1-H))                      !seb CHANGED Dc(Icell(ic,ir-1,il),3) to Dc(JA(I),3)
              BB(ij) = BB(ij) + A(I)*Hcp1
              sum = sum + A(I)
            ELSEIF ( irr.GT.ir ) THEN
              A(I) = (Ccc  + Dc(JA(I),2)*(Hrp1-H))                      !seb CHANGED Dc(Icell(ic,ir-1,il),2) to Dc(JA(I),2)
