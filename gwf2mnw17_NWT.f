...
c
      MODULE GWFMNW1MODULE
        ...
        CHARACTER(LEN=32),SAVE,DIMENSION(:),    POINTER,CONTIGUOUS :: 
     +   MNWSITE
     ...
     +   HREF
      TYPE GWFMNWTYPE
        ...
        CHARACTER(LEN=32), DIMENSION(:),    POINTER,CONTIGUOUS:: MNWSITE
        INTEGER,           DIMENSION(:),    POINTER,CONTIGUOUS:: IOWELL2
        DOUBLE PRECISION,  DIMENSION(:,:),  POINTER,CONTIGUOUS:: WELL2
        DOUBLE PRECISION,  DIMENSION(:,:,:),POINTER,CONTIGUOUS:: HREF
      END TYPE
...
c     Arguments
c     ------------------------------------------------------------------
      INTEGER :: In, Iusip, Iude4, Iusor, Iupcg, Iulmg, Iugmg, Iupcgn,
     +           Iunwt, Igrid
...
c
      ...
      IWELPT = 0
      IF ( ierr.LE.1 ) IWELPT = IFRL(rn(3))                       !changed test from EQ.1 to LE.1 --rth
      NOMOITER = 9999
...
c
 ...
 9006 FORMAT (' Flow rates will not be estimated after the', i5, 
     +        'th iteration')
c
...
c    Test for if well is in active grid ......
          iok = 1
          IF ( i.GT.NCOL .OR. j.GT.NROW .OR. node.GT.NODES )THEN        !SCOTT STRANGE, NO CHECK FOR OUT OF BOUNDS NLAY
            iok = 0
            IF(LSTCHK(1))WRITE(IOUT,'(A,/,3(A,I7),I7,/,2(A,I7))')
     + '***MNW1 ERROR*** WELL OUT OF BOUNDS','READ IN ROW ',j,
     + ' COL ',i,' with a max Row, Col being ',NROW,NCOL,
     + ' The node is stored on node ',node, 
     + ' with the max number of nodes being ', NODES
          END IF
          drytest = HNEW(i,j,k) - HDRY
          IF (iok.GT.0 .AND. ABS(drytest).GT.ZERO25) THEN
            iok = IBOUND(i,j,k)
            IF(IBOUND(i,j,k).EQ.0)THEN
              IF(LSTCHK(1))WRITE(IOUT,'(3(A,I7),A)')
     +    '***MNW1 ERROR*** MNW node is specified at ROW ',j,' COL ',i,
     +    ' Layer ',k,' which is an inactive model cell (IBOUND=0)'
            END IF
          END IF
c
...
c
      IF ( IWELPT.EQ.0 ) THEN
        IF ( nqreject.GT.0 ) THEN
          CALL USTOP('***MNW1 ERROR***'//
     +      ' SEE LST FILE FOR TRANSCRIPT OF BAD MNW1 WELLS')
C          txt = ' wells were outside of the model domain.'
C          IF(LSTCHK(3)) THEN
C            WRITE (IOUT, '(1X,/,5x,i5,a50)') nqreject, txt
C          txt = 'The rejected pumpage totaled: '
C            WRITE (IOUT, '(1X,/1X,a34,g14.5)') txt, qreject
C          ENDIF
        ENDIF
c
        IF(LSTCHK(3)) THEN
...
c    Well will be simulated as a specified rate or GHB
            ...
            IF ( (qdes-qact)**2.GT.SMALL ) iqslv = 1                    !SCOTT SHOULD CHANGE TO ABS(qdes-qact)**2 TO PREVENT POTENTIAL NEGATIVE BASE
...
c
            qwbar = qwsum
            IF ( qsum**2.GT.ZERO25 ) qwbar = qwsum/qsum                 !SCOTT SHOULD CHANGE TO ABS(qsum)**2 TO PREVENT POTENTIAL NEGATIVE BASE
            DO m2 = m, NWELL2
              igrp2 = IFRL(WELL2(9, m2))
...
c -----print the summed rates if requested(IWL2CB<0).
            qwbar = WELL2(4, ne)
            IF ( qwfsum**2.GT.ZERO25 ) qwbar = qwsum/qwfsum             !SCOTT SHOULD CHANGE TO ABS(qwfsum)**2 TO PREVENT POTENTIAL NEGATIVE BASE
            href = WELL2(8, ne)
            hwell = WELL2(10, ne)
            dd = hwell - href
            IF(LSTCHK(3)) THEN
...
c   set thickness / conductance to 0 if cell is dry
        IF ( (HNEW(Ix,Iy,Iz)-HDRY)**2.0D0.LT.ZERO25 ) thick = 0.0D0     !SCOTT SHOULD CHANGE TO ABS(HNEW(Ix,Iy,Iz)-HDRY)**2 TO PREVENT POTENTIAL NEGATIVE BASE
        txx = tempKX*thick
