      MODULE GWFDRNMODULE
        ...
        CHARACTER(LEN=16),SAVE, DIMENSION(:), POINTER,CONTIGUOUS::DRNAUX
        REAL,             SAVE, DIMENSION(:,:),POINTER,CONTIGUOUS::DRAI
      TYPE GWFDRNTYPE
        INTEGER,POINTER  ::NDRAIN,MXDRN,NDRNVL,IDRNCB,IPRDRN
        INTEGER,POINTER  ::NPDRN,IDRNPB,NNPDRN
        CHARACTER(LEN=16), DIMENSION(:),   POINTER,CONTIGUOUS::DRNAUX
        REAL,              DIMENSION(:,:), POINTER,CONTIGUOUS::DRAI
      END TYPE
...
C6------RETURN
      ...
      RETURN
      END
      SUBROUTINE GWF2DRN7RP(IN,IGRID,IUNITNWT)
C     ******************************************************************
...
C6-------IF THE CELL IS EXTERNAL SKIP IT.
      ...
      EL = DRAI(4,L)
      IF ( EL.LT.BOT .and. IUNITNWT.gt.0 ) THEN
        IF(LSTCHK(1)) THEN
...
C-------SUBROUTINE GWF2DRT7AD
      SUBROUTINE GWF2DRN7AD(KSTP,Igrid)
C     *****************************************************************
...
C3---------IF THE CELL IS INTERNAL GET THE DRAIN DATA and subtract the displacement from that cell.
          DRAI(4,L)=DRAI(4,L) - DVZ(IC,IR,IL)
100     CONTINUE
      END IF
      RETURN
      END SUBROUTINE
...
C  Deallocate DRN MEMORY
      USE GWFDRNMODULE
C
      DEALLOCATE(GWFDRNDAT(IGRID)%NDRAIN)
      DEALLOCATE(GWFDRNDAT(IGRID)%MXDRN )
      DEALLOCATE(GWFDRNDAT(IGRID)%NDRNVL)
      DEALLOCATE(GWFDRNDAT(IGRID)%IDRNCB)
      DEALLOCATE(GWFDRNDAT(IGRID)%IPRDRN)
      DEALLOCATE(GWFDRNDAT(IGRID)%NPDRN )
      DEALLOCATE(GWFDRNDAT(IGRID)%IDRNPB)
      DEALLOCATE(GWFDRNDAT(IGRID)%NNPDRN)
      DEALLOCATE(GWFDRNDAT(IGRID)%DRNAUX)
      DEALLOCATE(GWFDRNDAT(IGRID)%DRAI  )
C
