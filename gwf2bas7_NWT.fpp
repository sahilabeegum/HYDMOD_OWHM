      MODULE GLOBAL
        USE XY_GRID_COORDINATE_INTERFACE
        PARAMETER(NIUNIT=100)
        CHARACTER(4),DIMENSION(NIUNIT),SAVE:: CUNIT                     !seb MOVED CUNIT TO GLOBAL MODULE
        INTEGER,PARAMETER:: DEFAULT_KIND=KIND(1D0)                      !seb KIND NUMBER THAT REPRESENTS SINGLE PRECISION                           !seb KIND NUMBER THAT REPRESENTS DOUBLE PRECISION
        INTEGER, SAVE, POINTER    ::LSTLVL
        ...
        LOGICAL, SAVE, POINTER    ::SUBLNK                              !seb VARIABLE THAT INDICATES THAT SUBLINK IS ACTIVE. IT IS SET TO FALSE BY DEFAULT AND ALTERED BY SUBPACKAGE
        INTEGER, SAVE,    DIMENSION(:),    POINTER,CONTIGUOUS ::IUNIT(:)
        DOUBLE PRECISION, SAVE, DIMENSION(:,:,:), POINTER,CONTIGUOUS::
     +                                                              HNEW
        INTEGER, SAVE,    DIMENSION(:),     POINTER,CONTIGUOUS ::LBOTM
        INTEGER, SAVE,    DIMENSION(:),     POINTER,CONTIGUOUS ::LAYCBD
        INTEGER, SAVE,    DIMENSION(:),     POINTER,CONTIGUOUS ::LAYHDT
        INTEGER, SAVE,    DIMENSION(:),     POINTER,CONTIGUOUS ::LAYHDS
        REAL,    SAVE,    DIMENSION(:),     POINTER,CONTIGUOUS ::PERLEN
        INTEGER, SAVE,    DIMENSION(:),     POINTER,CONTIGUOUS ::NSTP
        REAL,    SAVE,    DIMENSION(:),     POINTER,CONTIGUOUS ::TSMULT
        INTEGER, SAVE,    DIMENSION(:),     POINTER,CONTIGUOUS ::ISSFLG
        REAL,    SAVE,    DIMENSION(:),     POINTER,CONTIGUOUS ::DELR
        REAL,    SAVE,    DIMENSION(:),     POINTER,CONTIGUOUS ::DELC
        REAL,    SAVE,    DIMENSION(:,:,:), POINTER,CONTIGUOUS ::BOTM
        REAL,    SAVE,    DIMENSION(:,:,:), POINTER,CONTIGUOUS ::HOLD
        INTEGER, SAVE,    DIMENSION(:,:,:), POINTER,CONTIGUOUS ::IBOUND
        !INTEGER, SAVE,    DIMENSION(:,:,:), POINTER,CONTIGUOUS ::WETCEL
        REAL(DEFAULT_KIND),SAVE,DIMENSION(:,:,:),POINTER,CONTIGUOUS ::CR
        REAL(DEFAULT_KIND),SAVE,DIMENSION(:,:,:),POINTER,CONTIGUOUS ::CC
        REAL(DEFAULT_KIND),SAVE,DIMENSION(:,:,:),POINTER,CONTIGUOUS ::CV
        DOUBLE PRECISION,SAVE,DIMENSION(:,:,:),POINTER,CONTIGUOUS ::HCOF
        DOUBLE PRECISION,SAVE,DIMENSION(:,:,:),POINTER,CONTIGUOUS ::RHS
        REAL,    SAVE,    DIMENSION(:,:,:), POINTER,CONTIGUOUS::BUFF
        REAL,    SAVE,    DIMENSION(:,:,:), POINTER,CONTIGUOUS::STRT
        REAL,    SAVE,    DIMENSION(:,:,:), POINTER,CONTIGUOUS::DDREF
        TYPE (XY_GRID_COODINATES),SAVE,POINTER:: XYGRID                 !seb
      TYPE GLOBALTYPE
        INTEGER,POINTER    :: LSTLVL
        ...
        INTEGER,    DIMENSION(:),     POINTER,CONTIGUOUS ::IUNIT
        DOUBLE PRECISION, DIMENSION(:,:,:), POINTER,CONTIGUOUS ::HNEW
        INTEGER,    DIMENSION(:),     POINTER,CONTIGUOUS ::LBOTM
        INTEGER,    DIMENSION(:),     POINTER,CONTIGUOUS ::LAYCBD
        INTEGER,    DIMENSION(:),     POINTER,CONTIGUOUS ::LAYHDT
        INTEGER,    DIMENSION(:),     POINTER,CONTIGUOUS ::LAYHDS
        REAL,       DIMENSION(:),     POINTER,CONTIGUOUS ::PERLEN
        INTEGER,    DIMENSION(:),     POINTER,CONTIGUOUS ::NSTP
        REAL,       DIMENSION(:),     POINTER,CONTIGUOUS ::TSMULT
        INTEGER,    DIMENSION(:),     POINTER,CONTIGUOUS ::ISSFLG
        REAL,       DIMENSION(:),     POINTER,CONTIGUOUS ::DELR
        REAL,       DIMENSION(:),     POINTER,CONTIGUOUS ::DELC
        REAL,       DIMENSION(:,:,:), POINTER,CONTIGUOUS ::BOTM
        REAL,       DIMENSION(:,:,:), POINTER,CONTIGUOUS ::HOLD
        INTEGER,    DIMENSION(:,:,:), POINTER,CONTIGUOUS ::IBOUND
        !INTEGER,    DIMENSION(:,:,:), POINTER,CONTIGUOUS ::WETCEL
        REAL(DEFAULT_KIND),DIMENSION(:,:,:), POINTER,CONTIGUOUS ::CR
        REAL(DEFAULT_KIND),DIMENSION(:,:,:), POINTER,CONTIGUOUS ::CC
        REAL(DEFAULT_KIND),DIMENSION(:,:,:), POINTER,CONTIGUOUS ::CV
        DOUBLE PRECISION,DIMENSION(:,:,:), POINTER,CONTIGUOUS ::HCOF
        DOUBLE PRECISION,DIMENSION(:,:,:), POINTER,CONTIGUOUS ::RHS
        REAL,       DIMENSION(:,:,:), POINTER,CONTIGUOUS ::BUFF
        REAL,       DIMENSION(:,:,:), POINTER,CONTIGUOUS ::STRT
        REAL,       DIMENSION(:,:,:), POINTER,CONTIGUOUS ::DDREF
        TYPE (XY_GRID_COODINATES),POINTER:: XYGRID                      !seb
      END TYPE GLOBALTYPE
      TYPE(GLOBALTYPE),SAVE ::GLOBALDAT(10)
ccrth Added Variables for opeing other new standard output files such as MULT, PSIRAMP, etc.
ccrth      C-------DERIVED DATA TYPE VARIABLES
ccrth      TYPE LSTOUTPUT                         !HOLD UNIT NUMBERS AND FILE NAMES OF EXTRA FMP I/O
ccrth        INTEGER,         DIMENSION(14)::UNIT !***NOTE THAT THE DIM IS HARD WIRED***
ccrth        CHARACTER(20),   DIMENSION(14)::NAME !***NOTE THAT THE DIM IS HARD WIRED***
ccrth      END TYPE
ccrth      TYPE(LSTOUTPUT),SAVE,POINTER:: LSTOUT
C
ccrth      TYPE LSTTYPE
ccrth
      CONTAINS
C     ******************************************************************
C     ROUTINE TO CHECK WHETHER OUTPUT SHOULD BE WRITTEN TO THE LIST FILE
C     SRP (10/23/2012)
C     ******************************************************************
      LOGICAL FUNCTION LSTCHK(MTYPE)
C        SPECIFICATIONS:
C     ------------------------------------------------------------------
      !USE GLOBAL,       ONLY:LSTLVL  !seb moved routine into BAS MODULE GLOBAL
C        LSTLVL - DISPLAY MODE FOR THE LIST FILE
C           1 - DISABLE LIST FILE
C           2 - SHOW ERROR MESSAGES ONLY
C           3 - SHOW ERROR AND WARNING MESSAGES
C           4 - SHOW ERROR, WARNING, AND INFORMATIONAL MESSAGES (SHOW ALL)
C     ------------------------------------------------------------------
C       ARGUMENTS:     MTYPE (INTEGER) - TYPE OF MESSAGE TO BE WRITTEN
C                         1 - ERROR MESSAGE
C                         2 - WARNING MESSAGE
C                         3 - STATUS/INFORMATIONAL MESSAGE
C       RETURN VALUE:  (LOGICAL)
C                         TRUE - WRITE OUTPUT TO LIST FILE
C                         FALSE - DO NOT WRITE OUTPUT TO LIST FILE
C     ------------------------------------------------------------------
        INTEGER, INTENT(IN) :: MTYPE
C     ------------------------------------------------------------------

        IF(LSTLVL.GT.MTYPE) THEN
            LSTCHK=.TRUE.
        ELSE
            LSTCHK=.FALSE.
        ENDIF
...
C        PARAMETER (MXPAR=2000,MXCLST=2000000,MXINST=50000)             !wschmid: changed to 2000000 consistent with FMP2.1 release
        ...
        INTEGER,SAVE,POINTER ::PROPPRINT                                !seb >0 PRINT OUT GEOLOGIC PROPERTIES AFTER APPLYING MULT/PVAL/DEFAULT IS 0
      ...
      TYPE PARAMTYPE
        ...
        INTEGER,POINTER  ::PROPPRINT                                    !seb >0 PRINT OUT GEOLOGIC PROPERTIES AFTER APPLYING MULT/PVAL/DEFAULT IS 0
      ...
      MODULE GWFBASMODULE
        ...
        INTEGER,           SAVE, POINTER                 ::PDIFFPRT     !seb FLAG FOR PRINTING WARNING WHEN BUDGET PERCENT ERROR>PDIFFPRT
...
C
      ...
      ALLOCATE(PROPPRINT,PDIFFPRT,REALTIM)                              !seb
      PDIFFPRT=50                                                       !IF PERCENT ERROR GOES ABOVE 50% THEN PRINT TO CMD PROMPT
      IF(IGRID.EQ.1)THEN
        ALLOCATE(SUBLNK)
        SUBLNK=.FALSE.
      END IF
   ...
   26    FORMAT (1X,'THE FREE FORMAT OPTION HAS BEEN SELECTED')
    ...
    8    FORMAT(1X,'When solver convergence criteria are not met,',/
     ...
     2    1X,'discrepancy is greater than:',F10.4)
      ELSE IF(LINE(ISTART:ISTOP).EQ.'MAXPARAM') THEN                    !seb ADD NEW OPTION TO SPECIFY MAXIMUM PARAMETERS TO STORE
         IF(LSTCHK(3)) WRITE(IOUT,'(3(/,1x,A))')
     ...
     +          'THESE VALUSE MUST BE >0, SET TO 1 IF NOT USED'
         CALL URWORD(LINE,LLOC,ISTART,ISTOP,2,MXPAR,R,IOUT,INBAS)
         CALL URWORD(LINE,LLOC,ISTART,ISTOP,2,MXCLST,R,IOUT,INBAS)
         CALL URWORD(LINE,LLOC,ISTART,ISTOP,2,MXINST,R,IOUT,INBAS)
         IF(MXPAR<1)THEN
     ...
     +                    'WARNING MXINST<1; VALUE CHANGED TO MXINST=1'
             MXINST=1
         END IF
      ELSE IF(LINE(ISTART:ISTOP).EQ.'PERCENTERROR') THEN                !seb ADD NEW OPTION TO SPECIFY PRINT OUT OF ERRORS THAT EXCEDE A MINIMUM PRECENT
         CALL URWORD(LINE,LLOC,ISTART,ISTOP,2,PDIFFPRT,R,IOUT,INBAS)
         IF(LSTCHK(3)) WRITE(IOUT,'(1x,4A)')
     +        'RATE PERCENT ERROR WILL BE PRINTED FOR EVERY TIME STEP ',
     +        'THAT EXCEDES ',NUM2STR(PDIFFPRT),'%'
      END IF
      IF(LLOC.LT.700) GO TO 25
...
C CHECK FOR VALID MODEL DIMENSIONS NOW THAT IBOUND,DELR,DELC,BOTM HAS BEEN ESTABLISHED seb
      DIMTOL=1E-5
      CALL CHECK_FOR_VALID_DIMENSIONS(DIMTOL)
C
...
C seb PRINT OUT MAX PARAMETER USAGE
      IF(KPER.EQ.1.AND.LSTCHK(3)) THEN
        WRITE(IOUT,'(/,A,/,A,3(/,A,I10,A,I10),/,A,/,A)')
...
C     ------------------------------------------------------------------
      ...
      USE GWFBASMODULE,ONLY:DELT,TOTIM,PERTIM,REALTIM
C     ------------------------------------------------------------------
      CALL SGWF2BAS7PNT(IGRID)
C
...
C2------STRESS PERIOD(PERTIM).
      TOTIM=TOTIM+DELT
      PERTIM=PERTIM+DELT
      IF(REALTIM.GE.0D0)THEN !.AND.ISSFLG(KPER).EQ.0)                  !seb  ADDED POTENTIAL SKIPPING OF SS SP
         IF(ITMUNI.EQ.0 .OR. ITMUNI.EQ.4) THEN                          !DAYS
             REALTIM=REALTIM + DBLE(DELT) * 0.00273791D0
         ELSE IF(ITMUNI.EQ.1) THEN                                      !SECONDS
             REALTIM=REALTIM + DBLE(DELT) * 3.16888D-8
         ELSE IF(ITMUNI.EQ.2) THEN                                      !MINUTES
             REALTIM=REALTIM + DBLE(DELT) * 1.90133D-6
         ELSE IF(ITMUNI.EQ.3) THEN                                      !HOURS
             REALTIM=REALTIM + DBLE(DELT) * 0.00011408D0
         ELSE                                                           !YEARS
             REALTIM=REALTIM + DBLE(DELT)
         END IF
      END IF
C
...
C     ------------------------------------------------------------------
     ...
     1                      MSUM,VBVL,VBNM,IDDREF,IUBGT,PDIFFPRT
C     ------------------------------------------------------------------
C
      CALL SGWF2BAS7PNT(IGRID)
C
...
C4------PRINT TOTAL BUDGET IF REQUESTED
      IF(IBUDFL.NE.0) THEN !GO TO 120                               !seb CHANGED TO ALLOW PERCENT ERROR CHECK AT EVERY TIMESTEP
        CALL SGWF2BAS7V(MSUM,VBNM,VBVL,KSTP,KPER,IOUT,IUBGT,BUDPERC)
        IPFLG=1
      ELSE
        CALL SGWF2BAS7VNOPRT(MSUM,VBVL,BUDPERC)
      END IF
      IF(ABS(BUDPERC).GE.REAL(PDIFFPRT)) THEN
         WRITE(*,'(2A,F6.1)')'WARNING: ',
     +         'VOLUMETRIC RATE BUDGET PERCENT ERROR FOR TIME STEP IS ',
     +          BUDPERC,'%'
         IF(LSTCHK(2))WRITE(IOUT,'(/2A,F6.1,A/)')'WARNING: ',
     +         'VOLUMETRIC RATE BUDGET PERCENT ERROR FOR TIME STEP IS ',
     +          BUDPERC,'%'
         IF(IBUDFL.EQ.0)
     +      CALL SGWF2BAS7V(MSUM,VBNM,VBVL,KSTP,KPER,IOUT,IUBGT,BUDPERC)
      ELSEIF(PDIFFPRT.EQ.0.AND.IBUDFL.EQ.0) THEN
          CALL SGWF2BAS7V(MSUM,VBNM,VBVL,KSTP,KPER,IOUT,IUBGT,BUDPERC)
      END IF
C
...
C     ------------------------------------------------------------------
     ...
     +                     XY_GRID_COODINATES,XYGRID         !seb XY_GRID_COODINATES IS REQUIRED TO CALL ITS ALLOCATION FUNCTION
      USE GLOBAL,     ONLY: LSTCHK
...
C3------ITMUNI, and LENUNI from the line.
      ...
      CALL URWORD(LINE,LLOC,ISTART,ISTOP,3,K,R,-IOUT,INDIS)             !seb READ IN X COORD
     ...
     +  '         [ie NOT CELL CENTER]'
            CORNERCOORD=.TRUE.
            CALL URWORD(LINE,LLOC,ISTART,ISTOP,1,K,R,IOUT,INDIS)
        ELSE IF (LINE(ISTART:ISTOP).EQ.'PRINTCOORD') THEN
            IF(LSTCHK(3)) WRITE(IOUT,'(A)')
     ...
     1      ' -- STOP EXECUTION (SGWF2BAS7ARDIS)')
      ...
      IF(N.EQ.1) THEN                                                   !seb ADDED ABILIITY TO HAVE REAL TIME PASS THROUGH MODEL
        ...
        IF(LINE(ISTART:ISTOP).EQ.'STARTTIME')THEN
          CALL URWORD(LINE,LLOC,ISTART,ISTOP,3,I,R,IOUT,INDIS)
          REALTIM=DBLE(R)
        END IF
        ISTART=K
        ISTOP =KK
...
C--------------ALLOCATE AND BUILD COORDINATE SYSTEM OF MODEL AND OPTIONALLY PRINT seb
      !CURRENTLY COORDINATE SYSTEM NOT IN USE BY PACKAGES, ONLY CALCLATE WHEN USER REQUEST IT TO BE PRINTED REMOVE "IF(PRINTCOORD) THEN" WHEN REQUIRED ALL THE TIME seb
      IF(PRINTCOORD) THEN
     ...
     +                  LLCOODRINATE,CORNERCOORD,DELR,DELC)
      !
      IF(PRINTCOORD.AND.LSTCHK(3)) CALL XYGRID%PRINT(IOUT)
      ELSE
          ALLOCATE(XYGRID)
      END IF
C
...
C     ------------------------------------------------------------------
     ...
     3                        VBVL,VBNM,IDDREF,IDDREFNEW,IUBGT
...
C5------ERROR DECODING INPUT DATA.
 2000 IF(LSTCHK(1)) THEN
          WRITE(IOUT,2001) LINE
      ENDIF
...
C     ------------------------------------------------------------------
      ...
      USE GWFBASMODULE,ONLY:REALTIM
C     ------------------------------------------------------------------
      CHARACTER(9),EXTERNAL::MONTHPRINT
C
      IF(LSTCHK(3)) THEN
...
C5------PRINT TIME STEP LENGTH AND ELAPSED TIMES IN ALL TIME UNITS.
  ...
  220 FORMAT(19X,' SECONDS     MINUTES      HOURS',7X,
     1    'DAYS        YEARS     DECIMAL YEAR: ',F8.3,' (',A,')',
     2     /20X,59('-'))
C
C6------RETURN
      RETURN
      END
      SUBROUTINE SGWF2BAS7V(MSUM,VBNM,VBVL,KSTP,KPER,IOUT,IUBGT,BUDPERC)
C     ******************************************************************
...
C6------PRINT INDIVIDUAL OUTFLOW RATES AND VOLUMES AND THEIR TOTALS.
     ...
     1       (VBVL(4,L).GE.BIGVL1 .OR. VBVL(4,L).LT.SMALL)) THEN
      ...
      ENDIF
      If(IUBGT>0) Then
        Write(IUBGT,276)VBNM(L),VAL2
  276 FORMAT(A16,2x,A17)
      End If
  250 CONTINUE
...
C
  260 FORMAT('1',/2X,'VOLUMETRIC BUDGET FOR ENTIRE MODEL AT END OF'
     1,' TIME STEP',I5,' IN STRESS PERIOD ',I6/2X,78('-'))
...
C
      END
      SUBROUTINE SGWF2BAS7VNOPRT(MSUM,VBVL,BUDPERC)
C     ******************************************************************
...
C     ******************************************************************
C        MODIFICATIONS:
C          SRP (10/23/2012): ADDED FILE BUFFERING OPTION
C          SRP (10/23/2012): ADDED OPTION TO CONTROL OUTPUT LEVEL TO LIST FILE
C                             (ERROR, WARNING, INFORMATIONAL OUTPUT LEVELS)
C     ******************************************************************
C
C        SPECIFICATIONS:
C     ------------------------------------------------------------------
      USE GLOBAL,       ONLY:LSTLVL,LSTCHK
      ...
      CHARACTER*20 FILTYP
      LOGICAL LOP,BUFFILE
      INTEGER BUFBLOCKSIZE
C     ---------------------------------------------------------------
C
C1------INITIALIZE CONSTANTS.
      BUFBLOCKSIZE = 1048576
      INBAS=0
      NFILE=0
      IOUT=0
      DO 5 I=1,NIUNIT
...
C-------SRP 10/23/2012 - ADDITION OF LIST FILE OUTPUT LEVEL (ERROR/WARNING/INFORMATIONAL)
C-------SRP 10/23/2012 - SUPPORT ADDED FOR BUFFERED FILES
C-------CHECK FOR NEGATIVE UNIT NUMBER
      BUFFILE=.FALSE.
      IF(IU.LT.0) THEN
C-------NEGATIVE UNIT NUMBER INDICATES OPTIONAL FILE BUFFERING AND LIST FILE OUTPUT LEVEL
C-------PARAMETER EXISTS.  GET PARAMETER
         IF(FILTYP.EQ.'LIST') THEN
            CALL URWORD(LINE,LLOC,ISTART,ISTOP,2,LSTLVL,R,IOUT,INUNIT)

            IF(LSTLVL.LT.0) THEN
C-------ENABLE FILE BUFFERING
               BUFFILE=.TRUE.
               LSTLVL=ABS(LSTLVL)
            ENDIF

C-------OUTPUT LIST FILE OUTPUT LEVEL TO CONSOLE
            IF(LSTLVL.EQ.1)
     1        WRITE(*,*)'List file disabled.'
            IF(LSTLVL.EQ.2)
     1        WRITE(*,*)'List file output mode: Error messages only.'
            IF(LSTLVL.EQ.3)
     1        WRITE(*,*)'List file output mode: Error/warning messages.'
            IF(LSTLVL.GE.4)
     1        WRITE(*,*)'List file output mode: Error/warning/info',
     2          ' messages.'
            WRITE(*,*)' '
         ELSE
C-------ENABLE FILE BUFFERING
         ...
         IF(BUFFILE) THEN
            IF(LSTCHK(3)) THEN
               WRITE(IOUT,123) FNAME(1:IFLEN)
            ENDIF
  123       FORMAT(1X,'FILE BUFFERING ENABLED FOR ',A)
         END IF
      END IF
...
C6------SPECIAL CHECK FOR 1ST FILE.
      IF(NFILE.EQ.0) THEN
        IF(FILTYP.EQ.'LIST') THEN
          IOUT=IU
C-------SRP 10/23/12 - LIST FILE DISABLED WHEN LSTLVL = 1
          IF(LSTLVL.GT.1) THEN
C-------SRP 10/23/12 - FILE BUFFERING OPTION ADDED !seb ADDED MACRO TO CHECK IF BUFFERED IS SUPPORTED BY COMPILER
            IF (BUFFILE) THEN
#ifdef __INTEL_COMPILER
              OPEN(UNIT=IU,FILE=FNAME(1:IFLEN),STATUS='REPLACE',
     1          FORM='FORMATTED',ACCESS='SEQUENTIAL',BUFFERED='yes',
     2          BLOCKSIZE=BUFBLOCKSIZE)
#else
              OPEN(UNIT=IU,FILE=FNAME(1:IFLEN),STATUS='REPLACE',
...
C-------SRP 10/23/12 - FILE BUFFERING OPTION ADDED
      IF (BUFFILE) THEN
#ifdef __INTEL_COMPILER
        OPEN(UNIT=IU,FILE=FNAME(1:IFLEN),FORM=FMTARG,BUFFERED='yes',
     1      BLOCKSIZE=BUFBLOCKSIZE,ACCESS=ACCARG,STATUS=FILSTAT,
     2      ACTION=FILACT,ERR=2000)
#else
...
C seb
      USE ExpressionParser  !ONLY PROVIDES ACCESS TO EqnEval AND MLTLOC
C
      CHARACTER*20 RW
      CHARACTER*1 COP
      CHARACTER*24 ANAME
...
C1------Read Number of Zone Arrays if Zone Option is active.
      NZONAR=0
      IF(INZONE.GT.0) THEN
         IF(LSTCHK(3)) WRITE(IOUT,1) INZONE                             !seb CHANGED IOUTM TO IOUT FOR ENTIRE IF
    1    FORMAT(1X,/1X,'ZONE OPTION, INPUT READ FROM UNIT ',I4)
         ...
         IF(LSTCHK(3)) WRITE(IOUT,2) NZONAR
    2    FORMAT(1X,I5,' ZONE ARRAYS')
         IF(NZONAR.LT.0) NZONAR=0
C seb READ IN FLAG TO PRINT OUT ZONE ARRAYS
        CALL URWORD(LINE,LLOC,ISTART,ISTOP,2,ZONEPRINT,DUM,-1,INZONE)   !seb OPTIONAL READ IN OF PRINT FLAG >0 MEANS PRINT OUT
        IF (LINE(LEN(LINE):LEN(LINE)).EQ.'E') THEN
          IF(LSTCHK(3)) WRITE(IOUT,'(A,/,A)') 'ZONEPRINT NOT SPECIFIED',
...
C3------Read Number of Multiplier Arrays if Multiplier Option is active.
      NMLTAR=0
      IF(INMULT.GT.0) THEN
         IF(LSTCHK(3))WRITE(IOUT,11) INMULT                             !seb CHANGED IOUTM TO IOUT FOR ENTIRE IF
   11    FORMAT(1X,/1X,'MULTIPLIER OPTION, INPUT READ FROM UNIT ',I4)
         ...
         IF(LSTCHK(3))WRITE(IOUT,12) NMLTAR
   12    FORMAT(1X,I5,' MULTIPLIER ARRAYS')
         IF(NMLTAR.LT.0) NMLTAR=0
      END IF
C seb READ IN FLAG TO PRINT OUT MULTIPLIER ARRAYS
      IF(NMLTAR.GT.0)THEN
        CALL URWORD(LINE,LLOC,ISTART,ISTOP,2,MULTPRINT,DUM,-1,INMULT)   !seb OPTIONAL READ IN OF PRINT FLAG >0 MEANS PRINT OUT
        IF (LINE(LEN(LINE):LEN(LINE)).EQ.'E') THEN
          IF(LSTCHK(3)) WRITE(IOUT,'(A,/,A)') 'MULTPRINT NOT SPECIFIED',
...
C6E4----Lookup the operand in the list of existing multipliers
   ...
   50        CONTINUE
             IF(LSTCHK(3))WRITE(IOUT,51) LINE(ISTART:ISTOP)             !seb CHANGED IOUTM TO IOUT TO PRODUCE ERROR IN LIST FILE INSTEAD OF EXTRA FILE
   51        FORMAT(1X,
...
C     ------------------------------------------------------------------
      ...
      USE GLOBAL,    ONLY:LSTCHK
      USE PARAMMODULE, ONLY:MXPAR,IPSUM,PARNAM,B,NPVAL,PROPPRINT
C
...
C4------READ & PRINT NUMBER OF PARAMETER VALUES.
      ...
      IF(LSTCHK(3)) THEN
       WRITE (IOUT,14) NPVAL
       IF(PROPPRINT>0)WRITE(IOUT,'(A)')'PROPPRINT>0: PROPERTIES AFTER'//
     +  ' PARAMETERS HAVE BEEN APPLIED WILL BE WRITEN TO SEPARATE FILES'
      ENDIF
...
C  DEALLOCATE GLOBAL DATA
      ...
      USE GWFBASMODULE
C
        IF(IGRID.EQ.1) DEALLOCATE(SUBLNK)                               !seb NOTE THE SUBLNK VARIABLE IS INDEPENDENT OF THE LGR GRID
...
C
        CALL GLOBALDAT(IGRID)%XYGRID%CLEAR()
...
!      END SUBROUTINE
      ...
      LEAPYR=.FALSE.
      B=0.0005  !yr = ~4.4 hours  --Correction factor, if new month is four hours away, print out that one.

      IF(     (MOD(YEAR,4).EQ.0 .AND. MOD(YEAR,100).NE.0)            !LEAP YEAR IF YEAR IS DIVISABLE BY 4 AND NOT 100 OR IF YEAR IS DIVISABLE BY 400
     +   .OR.  MOD(YEAR,400).EQ.0                       )LEAPYR=.TRUE.
!ASSUMES 28 days in feb
      ...
      END FUNCTION
C
      SUBROUTINE CHECK_FOR_VALID_DIMENSIONS(DIMTOL)
      USE GLOBAL,     ONLY: IOUT, NCOL,NROW,NLAY,DELR,DELC,
     +                      BOTM,LBOTM,IBOUND,LSTCHK
      USE CVT2STR, ONLY: NUM2STR
      IMPLICIT NONE
      ...
      CHARACTER(1),  PARAMETER::NL=ACHAR(10) !CARRAGE RETURN
      CHARACTER(422),PARAMETER::HEADER_WARN = REPEAT('#',35)//
     +             ' MODEL DIMENSION WARNING ' // REPEAT('#',80)// NL //
     +             REPEAT('#',140)// NL //REPEAT('#',140)
      CHARACTER(424),PARAMETER::HEADER_FAIL= REPEAT('#',35)//
     +             ' MODEL DIMENSION FAILURE ' // REPEAT('#',80)// NL //
     +             REPEAT('#',140)// NL //REPEAT('#',140)
      KILLPROG=.FALSE.
      !
      IF(LSTCHK(3)) WRITE(IOUT,'(/ A /)')
     +'CHECKING FOR VALID MODEL DIMENIONS FOR DELR, DELC, AND THICKNESS'
      !
      MAXR=MAXVAL(DELR)
      MAXC=MAXVAL(DELC)
      MAXTHCK=MAXVAL(
     +      BOTM(:,:,LBOTM(1:NLAY)-1) - BOTM(:,:,LBOTM(1:NLAY)),
     +      MASK=IBOUND(:,:,:).NE.0 )
      MINR=DIMTOL*MAXR
      MINC=DIMTOL*MAXC
      ...
      PRINTHEAD=.TRUE.
      DO IR=1, NROW
          IF(ALL(IBOUND(:,IR,:).EQ.0)) CYCLE
          IF(DELC(IR) < MINC) THEN
              IF(LSTCHK(1).AND.PRINTHEAD) THEN
               WRITE(IOUT,'(A)')HEADER_WARN
               PRINTHEAD=.FALSE.
              END IF
              IF(LSTCHK(1)) WRITE(IOUT,'(/ 4A)')
     +        'WARNING: EXTREMELY SMALL DELC WITH VALUE OF ',
     +        NUM2STR(DELC(IR)),', LOCATED AT ROW: ',NUM2STR(IR)
          END IF
          IF(DELC(IR) .LE. 0.0) KILLPROG=.TRUE.
      END DO
      DO IC=1,NCOL
          IF(ALL(IBOUND(IC,:,:).EQ.0)) CYCLE
          IF(DELR(IC) < MINR) THEN
              ...
              IF(LSTCHK(1)) WRITE(IOUT,'(/ 4A)')
     +        'WARNING: EXTREMELY SMALL DELC WITH VALUE OF ',
     +        NUM2STR(DELR(IC)),', LOCATED AT COL: ',NUM2STR(IC)
           END IF
     ...
     +      'WARNING: EXTREMELY SMALL CELL THICKNESS WITH VALUE OF ',
     +        NUM2STR(THK),', LOCATED AT ROW, COL, LAY: ',
     +        NUM2STR(IR),', ',NUM2STR(IC),', ',NUM2STR(IL)
           END IF
      ...
      IF(KILLPROG)THEN
          ...
          WRITE(IOUT,'(/ 13X 2A /)')'ONE OR MORE MODEL CELLS HAVE A ',
     +                      'WIDTH OR LENGTH OR THICKNESS = 0 OR < 0'
          CALL USTOP('MODEL DIMENSION FAILURE')
      END IF
      !
      END SUBROUTINE
