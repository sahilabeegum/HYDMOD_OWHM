...
!      IUSUB : Use the SUBsidence package ? (1-yes, 0-no)
       REAL,  SAVE,  DIMENSION(:,:), POINTER, CONTIGUOUS :: SEEP
...
!     ******************************************************************
       ...
       USE GWFSEEPMODULE,ONLY:NSEEP,MXSEEP,NSPVL,ISPCB,IPRSP,NPSEEP,&
                             &ISPPB,NNPSP,SPAUX,SEEP,IUSUB
       IMPLICIT NONE
!      Parameters
...
!      Read ISPCB  - SeePage data output unit
       ...
       END IF
!      Allocate space for auxiliary variable names
       ALLOCATE(SPAUX(20))
       NAUX=0
       IPRSP=1
       IUSUB=0
...
!      5 input data values and 1 location for cell-by-cell flow
       NSPVL=6+NAUX
!      Allocate space for the seepage cells
       ISPPB=MXACTS+1
       MXSEEP=MXACTS+MXPS
...
!         Read instances
        ...
        END DO
       END IF
!      Save the seepage data in the desired grid
...
!      Parameters
       ...
       CHARACTER(*),PARAMETER :: FMT2 = "(1X,/1X,'THE NUMBER OF &
       &ACTIVE SEEPAGE CELLS (',I6,') IS GREATER THAN MXACTS(',I6,')')"
...
!      Determine the number of non-parameter seepage cells
       IF(ITMP.LT.0) THEN
        IF(LSTCHK(3)) WRITE(IOUT,FMT1)
       ELSE
        NNPSP=ITMP
       END IF
!      Read new non-parameter seepage cells if any
       MXACTS=ISPPB-1
       IF(ITMP.GT.0) THEN
        IF(NNPSP.GT.MXACTS) THEN
         ...
         CALL USTOP(' ')
		END IF
! This line modified by Adam Szymkiewicz, 23.03.2016
        CALL ULSTRD(NNPSP,SEEP,1,NSPVL,MXSEEP,1,IN,IOUT,HEADER,&
                   &SPAUX,20,NAUX,IFREFM,NCOL,NROW,NLAY,5,5,IPRSP)		
       END IF
       NSEEP = NNPSP
!      Update the active seepage parameters if any
       CALL PRESET('SEEP')
       IF(NP.GT.0)THEN
        NREAD = NSPVL - 1
        DO N=1,NP
         CALL UPARLSTSUB(IN,'SEEP',IOUTU,'SEEP',SEEP,NSPVL,MXSEEP,&
                        &NREAD,MXACTS,NSEEP,5,5,HEADER,SPAUX,20,NAUX)
        END DO
       END IF
...
!       However, we still need to make sure the SUB package is linked
        IF(SUBLNK) THEN
         DO L=1,NSEEP
          ...
          IR = SEEP(2,L)
          IC = SEEP(3,L)
!         Get seepage information where appropriate
...
! modified from double precision by Adam Szymkiewicz, 23.03.2016
       REAL  :: HHNEW,CEL,RATOUT,Q,C,EL,EEL
...
!     CHECK: Removed CC and QQ (deemed unnecessary)
...
! modified by Adam Szymkiewicz, 23.03.2016
		 IF(IBD.EQ.2) CALL UBDSVB(ISPCB,NCOL,NROW,IC,IR,IL,Q,&
                       &SEEP(:,L),NSPVL,NAUX,6,IBOUND,NLAY)
