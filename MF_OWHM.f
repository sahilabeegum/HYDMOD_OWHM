...
!     &  /,20X,'SWR1 Version ',A/)
      ...
      open (unit=97,file='Sumvbot')
      open(unit=77,file='Solute details')
...
! Allocate arrays for Newton Solver
     ...
     1                                   IUNIT(22),IGRID)
      ...
      IF(IUNIT(41).GT.0)CALL GWF2SEEP1AR(IUNIT(41),IGRID)
...
C ScottBoyce Moved GWF2HFB7AR to before GWF2BCF7AR
     ...
     1                                     IUNIT(22),IGRID)
      ...
      IF(IUNIT(69).GT.0) CALL GWF2UNSF1AR(IUNIT(69),IGRID,IUNIT(5),
     1                                    IUNIT(8))
...
cswm: note the '1' below is hardwired for the parent grid
        ...
        IF(IUNIT(41).GT.0)CALL GWF2SEEP1RP(IUNIT(41),IGRID,IUNIT(63))
...
ccrth--Update Drain Elevations if SUB-LINK active
        IF(IUNIT(54).GT.0)THEN
         ...
         IF(IUNIT(41).GT.0) CALL GWF2SEEP1AD(KKSTP,IGRID)
...
ccrth
 ...
 1000         FORMAT(/1X,
     ...
     &      ' -- STOP EXECUTION')
          ...
          END IF
          IF(IUNIT(69).GT.0) CALL GWF2UNSF1AD(IGRID,KKSTP,KKPER)
...
C7C2A---FORMULATE THE FINITE DIFFERENCE EQUATIONS.
     ...
     1                             KKSTP,KKPER,IUNIT(47),ILGR,IGRID)
            ...
            IF(IUNIT(41).GT.0)CALL GWF2SEEP1FM(IGRID)
     ...
     1                                                    1,IGRID)
            ...
            IF(IUNIT(69).GT.0) CALL GWF2UNSF1FM(IGRID)
...
159         CONTINUE
          ...
          IF(IUNIT(41).GT.0)CALL GWF2SEEP1BD(KKSTP,KKPER,IGRID)
     ...
     1                                                     1,IGRID)
          ...
          IF(IUNIT(69).GT.0)CALL GWF2UNSF1BD(IGRID,KKSTP,KKPER)
...
C9------LAST BECAUSE IT DEALLOCATES IUNIT.
      ...
      IF(IUNIT(41).GT.0)CALL GWF2SEEP1DA(IGRID)
      ...
      IF(IUNIT(69).GT.0) CALL GWF2UNSF1DA(IGRID)
...
C Modified by Adam Szymkiewicz, 30.03.2016
      IF(IUNIT(69).GT.0) CALL GWF2UNSF1FCLOSE()        
...
C10-----END OF PROGRAM.
	...
	!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
C Modified by Adam Szymkiewicz, 30.03.2016
C      IF(IUNIT(69).GT.0) CALL GWF2UNSF1FCLOSE()
      CALL USTOP(' ')
