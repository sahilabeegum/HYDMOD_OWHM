C
C ==> SFR2 Array Definitions
C
C       ISTRM(1,NSTRM)=layer number
C       ISTRM(2,NSTRM)=row number
C       ISTRM(3,NSTRM)=column number
C       ISTRM(4,NSTRM)=Stream Segment Number
C       ISTRM(5,NSTRM)=Reach number
C       STRM(1,NSTRM)=  Reach Length
C       STRM(2,NSTRM)=  Streambed slope
C       STRM(3,NSTRM)=  Streambed Top elevation 
C       STRM(4,NSTRM)=  Streambed Bottom Elevation
C       STRM(5,NSTRM)=  Streambed width
C       STRM(6,NSTRM)=  Streambed Hydraulic Conductivity
C       STRM(7,NSTRM)=  Maximum streambed depth above channel bottom
C       STRM(8,NSTRM)=  Streambed thickness
C       STRM(9,NSTRM)=  Streamflow out of reach
C       STRM(10,NSTRM)= Streamflow into reach
C       STRM(11,NSTRM)= Seepage across streambed
C       STRM(12,NSTRM)= Overland runoff into stream reach 
C       STRM(13,NSTRM)= Stream ET 
C       STRM(14,NSTRM)= Stream Precipitation 
C       STRM(15,NSTRM)= Stream head or stage (HSTR)
C       STRM(16,NSTRM)= Streambed Conductance
C       STRM(17,NSTRM)= Head Difference across streambed      
C       STRM(18,NSTRM)= Head gradient across streambed
C       STRM(19,NSTRM)= Head in aquifer beneath stream reach
C       STRM(20,NSTRM)= Stream Reach Wetted Perimeter
C       STRM(21,NSTRM)= Recharge from the unsaturated zone beneath stream reach (Totflwt/Delt)
C       STRM(22,NSTRM)= Change in unsaturated-zone storage beneath stream reach (Totdelstor/Delt)
C       STRM(23,NSTRM)= Unsaturated-zone storage beneath stream reach (Totuzstor)
C       STRM(24,NSTRM)= Runoff to stream from ground-water discharge and rejected infiltration
C       STRM(25,NSTRM)= STRM(9,l) for transient state
C       STRM(26,NSTRM)= STRM(10,l) for transient state
C       STRM(27,NSTRM)= STRM(25,l) for steady state
C       STRM(28,NSTRM)= STRM(26,l) for steady state
C       STRM(29,NSTRM)= Accumulator of total flow in unsaturated flow beneath reach => STRM(29,L) + Totdelstor
C       STRM(30,NSTRM)= Transient_bd from flow of Routing channel routine
C
C
C     ******************************************************************
...
C     ------------------------------------------------------------------
     ...
     +'$Id: gwf2sfr7_NWT.f 2359 2012-04-05 00:41:23Z rniswon $'
      ...
      ALLOCATE (NSFRAUX, SFRAUX(20))                                    !swm - LGR added by wschmid
...
! Check keyword for tabular inflow rates.
      CALL UPARLSTAL(IN,IOUT,LINE,NPP,MXVL)
      !
      ALLOCATE(SFRTABFILE)                                              !seb ADDED TABFILE SUPPORT FROM TABFILE_INTERFACE
      ...
      IF(LINE(ISTART:ISTOP).EQ.'TABFILES') THEN
         IF(LSTCHK(3))WRITE(IOUT,'(3A)')'"TABFILES" OPTION SELECTED ',  !seb ADDED WARNING NOTICE
     +   'THIS IS THE ORIGINAL FEATURE WITH SFR2, WHICH IS DIFFERENT ',
     +   'FROM THE NEW "TABFILE" OPTION' 
   ...
   31    FORMAT(1X,I10,' Specifed inflow files will be read ',
     +                 'with a maximum of ',I10,' row entries per file')
         READ(IN,'(A)') LINE
      ELSE
         IF(LSTCHK(3).AND.SFRTABFILE%NTAB.EQ.0) THEN                    !seb
...
C
C insert from swm - LGR - added by wschmid
...
!!      DVRPERC = 0.0   !cjm
     ...
     & IDIVAR(2,nsegdim))    !wschmid increased from 26 to 27 to save seg(13
!      ALLOCATE (IDVFLG)    !disabled by rgn
!      IDVFLG = 0           !disabled by rgn
...
C
     ...
     +    CALL SGWF2SFR7UHC(Iunitlpf, Iunithuf, Iunitupw)
      END IF
      !
      CALL TABFILEPACKINDEX(SFRTABFILE,INDEXLST=[(I, I=1, NSS)])        !SEGMENT ROW LOCATION (ISEG) NEVER CHANGE IN SIMULATION, SO BUILD INDEX IN AR. NORMALLY DONE AT RP  ORIGINALLY, [1:NSS], BUT NOT SUPPORTED YET BY GFORTRAN
C
...
C-------SUBROUTINE GWF2SFR7RP
      SUBROUTINE GWF2SFR7RP(In, Iunitgwt, Iunitlak, Iunitsub, Kkper, 
     +                      Kkstp, Nsol,Iouts, Iunitbcf, Iunitlpf, 
     +                      Iunithuf, Iunitupw, Iunituzf, Igrid)
C     ******************************************************************
...
C     ------------------------------------------------------------------
     ...
     +     wdthlw, width, updiff, zero, DDIFF
     ...
     +        Ltyp,ILP,ierr,IFLG !WSCHMID/RTH
C     ------------------------------------------------------------------
      ALLOCATABLE DDIFF(:)
C
...
C9A-----CHECK FOR ERROR WHEN USING "SLOPE"-OPTION, IRDFLG=2
          IF(ABS(IRDFLG).EQ.2 .AND. IDIVAR(1,NSEG).GT.0                          ! WSCHMID 03/17/06 RTH
     +       .AND.(NSEG.EQ.NSS.OR.NSEG.EQ.1) ) THEN                              ! WSCHMID 01/30/06 RTH
          IF(LSTCHK(1)) THEN
              WRITE(IOUT, 9000)                                                     ! WSCHMID 01/30/06 RTH
          ENDIF
 9000     FORMAT (//1X, '***ERROR***  A DIVERSION SEGMENT CANNOT BE ',        ! WSCHMID 01/30/06 RTH
     +          'THE FIRST OR LAST SEGMENT, IF IRDFLG = 2',/1X,                 ! WSCHMID 01/30/06 RTH
     +          'PROGRAM IS STOPPING')                                          ! WSCHMID 01/30/06 RTH
            CALL USTOP(' ')                                                     ! WSCHMID 01/30/06 RTH
          ENDIF                                                                 ! WSCHMID 01/30/06 RTH
...
C
        END DO !end of loop for checking segment data --RTH
...
Crth --Include adjustment to BOTM if linked to SUB
      IF( ABS(IRDFLG).EQ.2 ) THEN
      ALLOCATE(DDIFF(NSS))      
         DO L = 2, NSTRM
          IF( ISTRM(4,L).GT.1 ) THEN
            IF( IDIVAR(1,ISTRM(4,L)-1).GT.0 ) THEN
              ilp = ISTRM(1,L-1)
              irp = ISTRM(2,L-1)              
              icp = ISTRM(3,L-1)
              IF(ISTRM(5,L).EQ.1 )  THEN
                IF(IUNITSUB.GT.0) THEN
                  IF(LPFLNK)THEN
                    SEG(13,ISTRM(4,L)-1)=SEG(13,ISTRM(4,L)-1)-
     1              DVZC(ICP,IRP,ILP)+DVZ(ICP,IRP,ILP)                   !subtract cum. displacement (but not last timestep's displacement: will be subtracted in AD routine)
                  ENDIF
                ENDIF
              DDIFF(ISTRM(4,L)-1)=BOTM(icp,irp,0)-SEG(13,ISTRM(4,L)-1)
              ENDIF
            ENDIF
          ENDIF
        ENDDO
...
C
C26-----COMPUTE STREAMBED TOP ELEVATION FOR REACHES OF CANAL DIVERSION SEGMENTS 
C       (OVERWRITE DEFAULT STREAMBED TOP ELEVATION) IN FARM PROCESS.
            seglen = SEG(1, nseg)
            sumlen = 0.0
            DO irch = 1, NSTRM
              IF( IDIVAR(1, ISTRM(4, irch)).EQ.IDIVAR(1, nseg) ) THEN
                ...
                IF( ISTRM(5, irch).EQ.1 ) THEN
                  IF(IUNITSUB.GT.0)THEN
                    IF(LPFLNK) THEN
                      SEG(8,ISTRM(4,IRCH))=
     1                SEG(8,ISTRM(4,IRCH))-DVZC(IC,IR,IL)+DVZ(IC,IR,IL) !subtract cum. displacement (but not last timestep's displacement: will be subtracted in AD routine)
                    ENDIF
                  ENDIF
                updiff = BOTM(ic,ir,0)-SEG(8,ISTRM(4,irch))
                ENDIF
                dndiff = DDIFF(ISTRM(4, irch))
...
C
            IF(IUNITSUB.GT.0)THEN
              IF(SUBLNK) GOTO 123                                       !NOT for sublnk: calculated in AD routine
            ENDIF
...
C       NOTE THAT FIRST AND LAST REACH CAN NOT BE DIVERSION-SEGMENT REACHES.
            DO irch = 2, NSTRM-1                                                !1ST AND LAST REACH CAN'T BE CANAL REACHES!!
              IF( IDIVAR(1, ISTRM(4, irch)).EQ.IDIVAR(1, nseg) ) THEN
...
! segment number, number of rows, unit number
     ...
     +                        TABFLOW(j,ISFRLIST(1,i))
        ...
        END IF
      END IF
 9034 FORMAT('A NEGATIVE VALUE FOR FLOW WAS SPECIFIED IN A ',
...
C-------SUBROUTINE GWF2SFR7AD
      SUBROUTINE GWF2SFR7AD(KSTP,Igrid)
C     *****************************************************************
C     APPLY EFFECTS OF SUBSIDENCE ON SW_GW INTERACTION AND CONVEYANCE
C     VERSION  7.2.00: July 7, 2010
C     *****************************************************************
      USE GWFSFRMODULE
      USE GLOBAL,       ONLY: IOUT,BOTM,LBOTM,SUBLNK
      USE GLOBAL,       ONLY:LSTCHK,SUBLNK
c      USE GWFBASMODULE, ONLY: DVZ
...
C     -----------------------------------------------------------------
     ...
     1                 fminelev, slopenew, dvzup, dvzrch, dvzdn, STRTOP
      INTEGER l, ic, ir, il, ilp, irp, icp, istsg, nreach, icu, iru, ilu
C     -----------------------------------------------------------------
      ALLOCATABLE STRTOP(:)
      !
      IF(SUBLNK)THEN
...
C
C2------SAVE ELEVATION OF STREAMBED TOP (BACKWARD DIFFERENCE dz3 NEEDS TO REFER TO UNADJUSTED ELEVATION)
        DO L = 1,NSTRM
          STRTOP(L)=DBLE(STRM(3,L))
        ENDDO
C3------DETERMINE LAYER, ROW, COLUMN OF EACH REACH.
        DO l = 1, NSTRM
          ...
          ir  = ISTRM(2,l)          
          ic  = ISTRM(3,l)
C3A-----FOR ANY REACH OTHER THAN LAST REACH OF A SEGMENT          
          IF(L.LT.NSTRM) THEN
            IF(ISTRM(5,L+1).NE.1) THEN
              ilp = ISTRM(1,l+1)
              irp = ISTRM(2,l+1)
              icp = ISTRM(3,l+1)
C3A1----APPLY INCREMENTAL DISPLACMENT IF STREAMBED ELEVATIONS ARE REUSED, PPESCRIBED, OR DERIVED FROM ALREADY DISPLACED BOTM ARRAY IN RP ROUTINE 
              if(ITMP.LT.0.OR.KSTP.GT.1.OR.ISFROPT.EQ.1.OR.
     1          (IRDFLG.EQ.2.AND.LPFLNK) ) THEN 
                DVZRCH=DVZ(IC,IR,IL)
                DVZDN=DVZ(ICP,IRP,ILP)
C3A2----APPLY CUMULATIVE DISPLACMENT SUPERIMPOSED ON DEFINED STREAMBED ELEVATIONS
              ELSE
                DVZRCH=DVZC(IC,IR,IL)
                DVZDN=DVZC(ICP,IRP,ILP)
              ENDIF
              dz1 = STRTOP(L)
              dz2 = STRTOP(L+1)
              dz3 = STRTOP(L)   - DVZRCH
              dz4 = STRTOP(L+1) - DVZDN
              dx3 = DBLE(STRM(1,L))   !wschmid
              dx4 = DBLE(STRM(1,L+1)) !wschmid
            ENDIF
          ENDIF
C3B-----FOR VERY LAST REACH (EQUAL TO NSTRM)
          IF(L.GT.1) THEN
            IF(L.EQ.NSTRM) THEN
              ilu = ISTRM(1,L-1)          
              iru = ISTRM(2,L-1)          
              icu = ISTRM(3,L-1)
C3B1----APPLY INCREMENTAL DISPLACMENT IF STREAMBED ELEVATIONS ARE REUSED, PPESCRIBED, OR DERIVED FROM ALREADY DISPLACED BOTM ARRAY IN RP ROUTINE           
              if(ITMP.LT.0.OR.KSTP.GT.1.OR.ISFROPT.EQ.1.OR.
     1          (IRDFLG.EQ.2.AND.LPFLNK) ) THEN 
                DVZUP=DVZ(ICU,IRU,ILU)
                DVZRCH=DVZ(IC,IR,IL)
C3B2----APPLY CUMULATIVE DISPLACMENT SUPERIMPOSED ON DEFINED STREAMBED ELEVATIONS                
              ELSE
                DVZUP=DVZC(ICU,IRU,ILU)
                DVZRCH=DVZC(IC,IR,IL)
              ENDIF         
              dz1 = STRTOP(L-1)
              dz2 = STRTOP(L)
              dz3 = STRTOP(L-1) - DVZUP
              dz4 = STRTOP(L)   - DVZRCH
              dx3 = DBLE(STRM(1,L-1)) !wschmid
              dx4 = DBLE(STRM(1,L))   !wschmid
C3C-----FOR LAST REACHES OF SEGMENTS OTHER THAN LAST SEGMENT
            ELSEIF(ISTRM(5,L+1).EQ.1) THEN
              ilu = ISTRM(1,L-1)          
              iru = ISTRM(2,L-1)          
              icu = ISTRM(3,L-1)
C3C1----APPLY INCREMENTAL DISPLACMENT IF STREAMBED ELEVATIONS ARE REUSED, PPESCRIBED, OR DERIVED FROM ALREADY DISPLACED BOTM ARRAY IN RP ROUTINE           
              if(ITMP.LT.0.OR.KSTP.GT.1.OR.ISFROPT.EQ.1.OR.
     1          (IRDFLG.EQ.2.AND.LPFLNK) ) THEN 
                DVZUP=DVZ(ICU,IRU,ILU)
                DVZRCH=DVZ(IC,IR,IL)
C3C2----APPLY CUMULATIVE DISPLACMENT SUPERIMPOSED ON DEFINED STREAMBED ELEVATIONS                
              ELSE
                DVZUP=DVZC(ICU,IRU,ILU)
                DVZRCH=DVZC(IC,IR,IL)
              ENDIF         
            ...
            ENDIF
          ENDIF
C          dz1 = DBLE(BOTM(ic,ir,LBOTM(il)-1))                          !can't use botm: double accounts for dvz subtraction if already done so in LPF
C          dz2 = DBLE(BOTM(icp,irp,LBOTM(ilp)-1))
C          dz3 = DBLE(BOTM(ic,ir,LBOTM(il)-1) - DVZ(ic,ir,il))
C          dz4 = DBLE(BOTM(icp,irp,LBOTM(ilp)-1) - DVZ(icp,irp,ilp))
C
C4-----DETERMINE CHANGE IN SLOPE
C         ALTERNATIVE: EITHER
C          if((dz1-dz2).lt.fminelev)then
C             dslope = (dz3 - dz4)/fminelev
C          else                            
C             dslope = (dz3 - dz4)/(dz1 - dz2)                           !careful here: dz3-dz4 can be negative, that is: dslope and slopenew would then be negative too
C          endif                              
C          slopenew = DBLE(STRM(2,l))*dslope                             !if slopenew of the previous timestep was = fminslope then the change in slope is going to be applied to fminslope again ... doesn't make much sense !
C         OR
          slopenew = (dz3-dz4) / (0.5*dx3+0.5*dx4)

c            write(iout,*) istsg,nreach,strm(2,l),slopenew,dslope,
c     1     dz1,dz2,dz3,dz4,fminelev

           STRM(2,l) = real(slopenew)
C
C5------DETERMINE DISPLACMENT OF STREAMBED TOP AND BOTTOM ELEVATION
C          STRM(3,l) = STRM(3,l) - DVZ(ic,ir,LBOTM(il))                !dvz is allocated and calculated for NLAY layers (not for nlay + attached conf. beds),
C          STRM(4,l) = STRM(4,l) - DVZ(ic,ir,LBOTM(il))                !so it should be sufficient and no lbotm(il) needed as it seems.
          STRM(3,l) = STRM(3,l) - REAL(DVZRCH)
          STRM(4,l) = STRM(4,l) - REAL(DVZRCH)
        END DO
C
        IF(ABS(IRDFLG).EQ.2) THEN
C27-----RE-COMPUTE STREAMBED SLOPE FOR REACHES OF DIVERSION SEGMENTS IF "SMOOTHING TO GROUND SURFACE" OPTION IS CHOSEN.
          DO L = 2, NSTRM-1
            IF( IDIVAR(1,ISTRM(4,L)).GT.0) THEN
              STRM(2,L)=(STRM(3,L-1)-STRM(3,L+1))/                      !central difference
     +        (0.5*STRM(1,L-1)+STRM(1,L)+0.5*STRM(1,L+1))
              IF( ISTRM(5,L).EQ.1) THEN                                 !forward difference
                STRM(2,L)=(STRM(3,L)-STRM(3,L+1))/
     1          (0.5*STRM(1,L)+0.5*STRM(1,L+1))
              END IF
              IF( ISTRM(5,L+1).EQ.1) THEN
                STRM(2,L)=(STRM(3,L-1)-STRM(3,L))/                      !backward difference
     1          (0.5*STRM(1,L-1)+0.5*STRM(1,L))
              END IF
c                write(iout,*) ISTRM(4,L),ISTRM(5,L),strm(2,l)
            ENDIF
          ENDDO
          DO L = 2, NSTRM-1
            IF( IDIVAR(1,ISTRM(4,L)).GT.0) THEN
              IF(STRM(2,L).LT.fminslope) THEN
               IF(LSTCHK(3)) THEN
                 write(iout,*) ISTRM(4,L),ISTRM(5,L),strm(2,l)           
               ENDIF
                STRM(2,L)=(STRM(2,L-1)+STRM(2,L)+STRM(2,L+1))/3         !moving average (period = 3)
c                 write(iout,*) ISTRM(4,L),ISTRM(5,L),strm(2,l)           
              ENDIF
            ENDIF
          ENDDO
          DO L = 2, NSTRM-1
            IF( IDIVAR(1,ISTRM(4,L)).GT.0) THEN
              IF(STRM(2,L).LT.fminslope) THEN
                IF(L.GT.2.AND.L.LT.NSTRM-1) THEN
c              write(iout,*) ISTRM(4,L),ISTRM(5,L),strm(2,l)
                  STRM(2,L)=(STRM(3,L-2)-STRM(3,L+2))/(0.5*STRM(1,L-2)+ !5-kernel central difference
     +            STRM(1,L-1)+STRM(1,L)+STRM(1,L+1)+0.5*STRM(1,L+2))
                ENDIF
                IF(L.LT.NSTRM-1.AND.ISTRM(5,L).EQ.1) THEN 
                  STRM(2,L)=(STRM(3,L)-STRM(3,L+2))/                    !asymetric 3-kernal forward difference
     1            (0.5*STRM(1,L)+STRM(1,L+1)+0.5*STRM(1,L+2))
                ENDIF
                IF(ISTRM(5,L).EQ.2) THEN
                  STRM(2,L)=(STRM(3,L-1)-STRM(3,L+1))/                  !3- kernal central difference
     1            (0.5*STRM(1,L-1)+STRM(1,L)+0.5*STRM(1,L+1))
                ENDIF                  
                IF(L.GT.2.AND.ISTRM(5,L+1).EQ.1) THEN
                  STRM(2,L)=(STRM(3,L-2)-STRM(3,L))/                    !asymetric 3-kernal backward difference
     1            (0.5*STRM(1,L-2)+STRM(1,L-1)+0.5*STRM(1,L))
                ENDIF
                IF(ISTRM(5,L+2).EQ.1) THEN
                  STRM(2,L)=(STRM(3, L-1)-STRM(3,L+1))/                 !3- kernal central difference
     1            (0.5*STRM(1,L-1)+STRM(1,L)+0.5*STRM(1,L+1))
                ENDIF
c              write(iout,*) ISTRM(4,L),ISTRM(5,L),strm(2,l)
...
C
        DO L = 1,NSTRM
          IF(STRM(2,L).LT.fminslope) THEN
            istsg = ISTRM(4,l)
            nreach = ISTRM(5,l)
            STRM(2,l) = real(fminslope)
            IF(LSTCHK(2)) THEN
              WRITE (iout,111) istsg, nreach, fminslope
            ENDIF
  111       format('***Warning*** Subsidence has caused a stream reach '
     +        /,'to have near zero or negative slope for Segment ',i5,
     +        ' and reach ',i5,/,' Resetting slope to ',1PG15.8)
          ENDIF
        ENDDO
      DEALLOCATE(STRTOP)
      !
...
C-------SET POINTERS FOR CURRENT GRID.
      ...
      IF(ILGR .NE. 0) CALL SGWF2NWT1PNT(IGRID)  !seb lgr
      IF(ILGR .NE. 0) CALL SGWF2BAS7PNT(IGRID)  !seb lgr
      IF(ILGR .NE. 0) CALL SGWF2LAK7PNT(IGRID)  !seb lgr
C
      iss = ISSFLG(Kkper)
      numdelt = NUMTIM
C
...
C         TO ZERO FOR LAKE PACKAGE.
      itstr = 0
      iprvsg = -1
      lgrgridprv = 0                   !swm - LGR added by wschmid      
C      fin = 0.0
C      fout = 0.0
      IF ( Iunitlak.GT.0 ) THEN
        DO i = 1, NSS
...
Cwschmid: inserted from LGR version
...
Cwschmid              IF ( Ilgr.EQ.0 .OR. Ilgr.EQ.1 .AND. ISCHILD.EQ.1) ) THEN
cwschmid                ilay = ilay + 1
              IF(ILGR.EQ.0) THEN
                ILAY = ILAY + 1
              ELSEIF(ILGR.EQ.1.AND.ISCHILD.EQ.1) THEN
                ILAY = ILAY + 1
...
C-------SUBROUTINE GWF2SFR7BD
     ...
     +           Iunituzf, Nsol, Iunitrch, Ilgr, Ngrids, Igrid)  !cjm (added Iunitrch)
C     *****************************************************************
...
C20-----SET FLOW INTO DIVERSION IF SEGMENT IS DIVERSION.
            ...
            END IF
C
C20B-----STORE OUTFLOW FROM PREVIOUS SEGMENT FOR RECHARGE  !cjm -- removed by rjn updated by rth
!!           IF ( istsg.GT.1 ) THEN
...
C25-----SEARCH FOR UPPER MOST ACTIVE CELL IN STREAM REACH. Revised ERB
          TOPCELL: DO ilay=il, NLAY                                     !seb changed loop to prevent ilay>NLAY
           IF ( IBOUND(ic, ir, ilay).NE.0 ) EXIT TOPCELL
          END DO TOPCELL
          IF(ilay.GT.NLAY) ilay=NLAY                                    !seb if ilay>NLAY then full column is inactive, set to NLAY so future checks will not get out of bound errors.
          IF ( IBOUND(ic, ir, ilay).NE.0) il = ilay
!          ilay = il
!          TOPCELL: DO WHILE ( ilay.LE.NLAY )
!            IF ( IBOUND(ic, ir, ilay).EQ.0 ) THEN
!              ilay = ilay + 1
...
C26-----DETERMINE LEAKAGE THROUGH STREAMBED.
 ...
 !               WETPER(1, l) = width
          ...
          END IF
C26a----CHECK IF LAKE INUNDATES STREAM. IF SO THEN SET avhc and cstr = 0.
          IF ( Iunitlak.GT.0 ) THEN                                     !SCOTT why are you doing illake = il-1? What about when il=1
            illake = il - 1
            IF ( illake.GE.1 ) THEN
              lakid = LKARR1(ic, ir, illake)
              IF ( lakid.GT.0 ) THEN
...
C         ZERO WHEN NOT AN ACTIVE CELL.
          IF ( IBOUND(ic, ir, ilay).LE.0 ) THEN                         !seb changed il to ilay
            avhc = 0.0
!           strleak = 0.0D0
            h = hstr
          END IF
          IF (IBOUND(ic, ir, ilay).GT.0) THEN                           !seb changed to follow ilay which is set to the last layer if all vertical model cells are IBOUND(i,j,:)=0
...
C29-----STREAMFLOW OUT EQUALS STREAMFLOW IN MINUS LEAKAGE.
          ELSE
            ...
            hdiff = 0.0D0
            h=HDRY
            hld = HDRY                                                  !seb ADDED h=HDRY and hld=HDRY because hld is located on incative cell, IBOUND(i,j,:)=0
          END IF
C
            ...
            IF ( flobot.GE.0.0D0 ) THEN
              ...
              IF ( runof.LT.NEARZERO ) THEN
                IF ( flowin+runoff+precip.LT.NEARZERO ) THEN
                  runof = 0.0D0
                  etstr = 0.0D0
C-WSCHMID/RTH--changed abs(runof) back to runof to accommodate negative runoff = farm diversion in FMP (05/07/08)                   
...
! RGN Fixed statement. Only call subroutine if no UZ flow.
          IF ( icalccheck.EQ.1 ) THEN                            
            IF ( h.LT.sbot .AND. IBOUND(ic, ir, ilay).NE.0) THEN        !seb ADDED  .AND. IBOUND(ic, ir, ilay).NE.0 to ensure that only active cells get UZ flow or set flobot=0
...
C37-----ADD RATES TO BUFFERS.
          IF ( icalccheck.EQ.1 .AND. iss.EQ.0 ) THEN
            IF ( irt.EQ.numdelt .AND. IBOUND(ic, ir, ilay).NE.0 ) THEN  !seb ONLY PROCESS ACTIVE CELLS
...
C39-----ADD FLOBOT TO RATIN WHEN STREAM RECHARGES GROUND WATER.
            IF ( flobot.GT.0.0D0 ) THEN
              ...
              SFRUZBD(8) = flobot
            END IF
            IF ( icalccheck.EQ.1 .AND. sbot.GT.h 
     +                            .AND. IBOUND(ic, ir, ilay).NE.0) THEN !seb ADDED  .AND. IBOUND(ic, ir, ilay).NE.0 to ensure that only active cells
              totflwt = flobot*DELT
            ...
            ELSE IF ( sbot.LT.h .AND. IBOUND(ic, ir, ilay).NE.0 ) THEN !seb  ADDED  .AND. IBOUND(ic, ir, ilay).NE.0 to ensure that only active cells
              totflwt = 0.0
            END IF
          END IF
C
...
C       WHEN STANDARD UNFORMATTED BUDGET. revised dep 5/10/2006
      IF ( ibdst.GT.0 ) THEN
        ...
        DO l = 1, NSTRM
          ...
          ic = ISTRM(3, l)
          ilay = il                                                     
          IF(IL.GT.NLAY) CYCLE                                          !seb IF TOP MOST LAYER WAS NOT FOUND THAN IL>NAY SO SKIP WRITING BUDGET SINCE THERE WAS NOT INTERACTION WITH GROUNDWATER
          IF(IL.EQ.NLAY .AND. IBOUND(IC, IR, IL).EQ.0) CYCLE            !seb IF TOP MOST LAYER IS NLAY AND IT HAS IBOUND=0 THAN ALL LAYERS ABOVE HAVE IBOUND=0 SO SKIP WRITING TO BUDGET SINCE THERE IS NO GROUNDWATER INTERACTION
...
!       IF ( n.GT.NSS .OR. n.LT.1 ) THEN  !cjm (commented this line out)
 ...
 9003       FORMAT (1X, 'READING ENTRY ', I6, ' OF ITEM 4A')
        ...
        END IF
C
C2a-----DETERMINE IF SEGMENT OUTFLOW WILL BE DIVERTED TO RECHARGE MF CELLS  !cjm Disabled rth
!!        IF ( N.LT.0 ) THEN
!!          N = ABS(N)
!!          DVRCH(N) = 1
!!        ELSE              !cjm 20090708
...
C         WATER TABLE.
        IF ( Numwaves.GT.1 ) THEN
          If(Speed(Jpnt+1).gt.0.)then
              bottomtime = (Depth(Jpnt)-Depth(Jpnt+1))/Speed(Jpnt+1) !Test for zero speed rth
          endif
...
C
!     -----------------------------------------------------------------
      SUBROUTINE GWF2SFR7LAKE(Iunitlak,IGRID)
C     ******************************************************************
...
C     ------------------------------------------------------------------
     ...
     +                        SEG, FXLKOT, IDIVAR, CLOSEZERO,SFRTABFILE !seb
      USE GLOBAL, ONLY: IOUT
      USE GLOBAL, ONLY:LSTCHK
      IMPLICIT NONE
      EXTERNAL FLOWTERP
      ...
      INTEGER i, iseg,KSTP, Iunitlak, istsg, lk,TABIDX,j,MX,LLOC,IGRID
C     ------------------------------------------------------------------
C
      CALL SGWF2SFR7PNT(Igrid)
...
C1a------CALL LINEAR INTERPOLATION ROUTINE
      ...
      CALL TABFILEUPDATE( SFRTABFILE,'SFR', SEG(2,:) )                  !seb UPDATES THE FLOW FOR ALL SEGMENTS LINKED TO A TABFILE
C
...
C     OF TIME TO CACULATE SPECIFIED INFLOW TO SEGMENTS.
     ...
     +                        CLOSEZERO
      ...
      I = 1
      TIMEBEG = TIME - DELT
      IF ( TIMEBEG-TABTIME(1,ISEG).LT.0.0 ) THEN                      !SHOULD THIS BE TOTIM NOT TIMBEG
...
! Find table value after end of time step
        ...
        END DO
        FLOWTERP = SUMFLOW/DELT   !SHOULD BE SUM(TIMEND-TIMESTART)_i seb
      END IF
      RETURN
...
C     ------------------------------------------------------------------
      ...
      DEALLOCATE (GWFSFRDAT(IGRID)%SFRTABFILE)                          !seb
C
C NULLIFY THE LOCAL POINTERS
      IF(IGRID.EQ.1) THEN
        ...
        SUMLEAK         =>NULL()
        HLDSFR          =>NULL()
        STRMDELSTOR_CUM =>NULL()
        STRMDELSTOR_RATE=>NULL()
        TOTSPFLOW       =>NULL()
        TABFLOW         =>NULL()
        TABTIME         =>NULL()
        ISFRLIST        =>NULL()
