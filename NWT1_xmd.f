...
cmi
     ...
     +                          IERR
      TYPE XMDTYPE
        LOGICAL, POINTER ::  REDSYS,LDCOMB
        DOUBLE PRECISION, POINTER ::  EPSRN,RRCTOL,HCLOSEXMD
        INTEGER, POINTER ::  MXITERXMD
        INTEGER, POINTER :: IACL,NORDER,LEVEL,NORTH,IDROPTOL,
     +                            IERR
      END TYPE
      TYPE(XMDTYPE),DIMENSION(10):: XMDDAT
      !
      CONTAINS
      !
      SUBROUTINE XMDPNT(IGRID)
        INTEGER::IGRID
        REDSYS    =>XMDDAT(IGRID)%REDSYS   
        LDCOMB    =>XMDDAT(IGRID)%LDCOMB   
        EPSRN     =>XMDDAT(IGRID)%EPSRN    
        RRCTOL    =>XMDDAT(IGRID)%RRCTOL   
        HCLOSEXMD =>XMDDAT(IGRID)%HCLOSEXMD
        MXITERXMD =>XMDDAT(IGRID)%MXITERXMD
        IACL      =>XMDDAT(IGRID)%IACL     
        NORDER    =>XMDDAT(IGRID)%NORDER   
        LEVEL     =>XMDDAT(IGRID)%LEVEL    
        NORTH     =>XMDDAT(IGRID)%NORTH    
        IDROPTOL  =>XMDDAT(IGRID)%IDROPTOL 
        IERR      =>XMDDAT(IGRID)%IERR     
      END SUBROUTINE
      SUBROUTINE XMDPSV(IGRID)
       INTEGER::IGRID
        XMDDAT(IGRID)%REDSYS    =>REDSYS    
        XMDDAT(IGRID)%LDCOMB    =>LDCOMB    
        XMDDAT(IGRID)%EPSRN     =>EPSRN     
        XMDDAT(IGRID)%RRCTOL    =>RRCTOL    
        XMDDAT(IGRID)%HCLOSEXMD =>HCLOSEXMD 
        XMDDAT(IGRID)%MXITERXMD =>MXITERXMD 
        XMDDAT(IGRID)%IACL      =>IACL      
        XMDDAT(IGRID)%NORDER    =>NORDER    
        XMDDAT(IGRID)%LEVEL     =>LEVEL     
        XMDDAT(IGRID)%NORTH     =>NORTH     
        XMDDAT(IGRID)%IDROPTOL  =>IDROPTOL  
        XMDDAT(IGRID)%IERR      =>IERR      
      END SUBROUTINE
