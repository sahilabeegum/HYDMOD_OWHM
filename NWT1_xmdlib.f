...
C seb
      ...
      IF(ALLOCATED(aqb))DEALLOCATE(aqb)
      IF(ALLOCATED(reso))DEALLOCATE(reso)
...
c
      ...
      ierror=0
      IF(ILGR.NE.0.AND.ALLOCATED(icolour))  THEN                        !seb lgr TO WORK WITH LGR
          IF(nn.GT.UBOUND(icolour,1)) THEN
      deallocate(icolour, RBorder, iblackend, lorder)
...
c
     ...
     [               iblackend, nn, nja, nblack, ierr, redsys)
      ...
      IF(ILGR.NE.0.AND.ALLOCATED(iaf))  THEN                        !seb lgr TO WORK WITH LGR
         IF(nblack+1.GT.UBOUND(iaf,1)) THEN
           deallocate(iaf, idiagf, stat = ierror )
