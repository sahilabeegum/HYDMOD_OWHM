...
C         SSWR_P_QMFLOW
C
C     VERSION 1.03 SWR1 for MODFLOW NWT AND OWHM
C     CHANGES
...
C         AUXILIARY VARIABLES
        ...
        CHARACTER(LEN=16),SAVE,DIMENSION(:),POINTER,CONTIGUOUS::SWRAUX
        REAL,SAVE,DIMENSION(:,:), POINTER,CONTIGUOUS  :: AUX
        REAL,SAVE,DIMENSION(:),   POINTER,CONTIGUOUS  :: AUXROW
C         DATASET 1A
...
C         SWR VARIABLES
        ...
        DOUBLEPRECISION,SAVE,DIMENSION(:,:), POINTER,CONTIGUOUS:: BFCNV
        INTEGER,SAVE,POINTER          :: ICALCQAQ
        INTEGER,SAVE,POINTER          :: NBDITEMS
        REAL,SAVE,DIMENSION(:,:), POINTER,CONTIGUOUS  :: CUMBD
        REAL,SAVE,DIMENSION(:,:), POINTER,CONTIGUOUS  :: INCBD
        REAL,SAVE,DIMENSION(:),   POINTER,CONTIGUOUS  :: CUMQAQBD
        REAL,SAVE,DIMENSION(:),   POINTER,CONTIGUOUS  :: INCQAQBD
...
C---------STORAGE MAPPING FOR COORDINATE INVARIANT FLOW (QM) IN MODEL-BASED CELLS 
        ...
        INTEGER,SAVE,DIMENSION(:,:), POINTER,CONTIGUOUS  :: IQMCI
        DOUBLEPRECISION,SAVE,DIMENSION(:,:), POINTER,CONTIGUOUS:: SFMCI
C---------VARIABLES ACCESSED OUTSIDE SWR - SIMPLE ARRAYS TO SIMPLIFY CALLS
        INTEGER,SAVE,POINTER          :: IEXTFLOW
        REAL,SAVE,DIMENSION(:,:,:),POINTER,CONTIGUOUS :: HK
C---------STORAGE OF REACH AND REACH GROUP STAGE
        DOUBLEPRECISION,SAVE,DIMENSION(:,:), POINTER,CONTIGUOUS:: RSTAGE
        DOUBLEPRECISION,SAVE,DIMENSION(:,:), POINTER,CONTIGUOUS:: GSTAGE
        DOUBLEPRECISION,SAVE,DIMENSION(:,:), POINTER,CONTIGUOUS :: DOBS
C---------DIRECT RUNOFF
        INTEGER,SAVE,DIMENSION(:,:),POINTER,CONTIGUOUS :: DROIMAP
        REAL,SAVE,DIMENSION(:,:),POINTER,   CONTIGUOUS :: DRORMULT
        REAL,SAVE,DIMENSION(:,:),POINTER,   CONTIGUOUS :: DRORVAL
C---------DERIVED DATA TYPES
        TYPE(TTABS),     SAVE,DIMENSION(:),POINTER,CONTIGUOUS :: TABDATA
        TYPE(TOBS),      SAVE,DIMENSION(:),POINTER,CONTIGUOUS :: OBSDATA
        TYPE(TREACH),    SAVE,DIMENSION(:),POINTER,CONTIGUOUS :: REACH
        TYPE(TRCHGRP),   SAVE,DIMENSION(:),POINTER,CONTIGUOUS :: RCHGRP
        TYPE(TJAC),      SAVE,             POINTER :: JAC
        TYPE(TSWRTIME),  SAVE,DIMENSION(:),POINTER,CONTIGUOUS :: SWRTIME
        TYPE(TSTABILITY),SAVE,POINTER :: FROUDE
C---------PRINT DATA
        INTEGER,SAVE,POINTER          :: NPMAX
        DOUBLEPRECISION,SAVE,DIMENSION(:,:),POINTER,CONTIGUOUS:: RSTAGEP
        DOUBLEPRECISION,SAVE,DIMENSION(:,:),POINTER,CONTIGUOUS:: GSTAGEP
        DOUBLEPRECISION,SAVE,DIMENSION(:),  POINTER,CONTIGUOUS:: DOBSP
...
C         AUXILIARY VARIABLES
          ...
          CHARACTER(LEN=16), DIMENSION(:), POINTER,CONTIGUOUS :: SWRAUX
          REAL,DIMENSION(:,:), POINTER,CONTIGUOUS  :: AUX
          REAL,DIMENSION(:),   POINTER,CONTIGUOUS  :: AUXROW
C
...
C         SWR VARIABLES
          ...
          DOUBLEPRECISION,DIMENSION(:,:), POINTER,CONTIGUOUS:: BFCNV
          INTEGER,POINTER          :: ICALCQAQ
          INTEGER,POINTER          :: NBDITEMS
          REAL,DIMENSION(:,:), POINTER,CONTIGUOUS  :: CUMBD
          REAL,DIMENSION(:,:), POINTER,CONTIGUOUS  :: INCBD
          REAL,DIMENSION(:), POINTER,CONTIGUOUS  :: CUMQAQBD
          REAL,DIMENSION(:), POINTER,CONTIGUOUS  :: INCQAQBD
...
C-----------STORAGE MAPPING FOR COORDINATE INVARIANT FLOW (QM) IN MODEL-BASED CELLS 
          ...
          INTEGER,DIMENSION(:,:), POINTER,CONTIGUOUS  :: IQMCI
          DOUBLEPRECISION,DIMENSION(:,:), POINTER,CONTIGUOUS:: SFMCI
C-----------VARIABLES ACCESSED OUTSIDE SWR - SIMPLE ARRAYS TO SIMPLIFY CALLS
          INTEGER,POINTER          :: IEXTFLOW
          REAL,DIMENSION(:,:,:),POINTER,CONTIGUOUS :: HK
C---------STORAGE OF REACH AND REACH GROUP STAGE
          DOUBLEPRECISION,DIMENSION(:,:), POINTER,CONTIGUOUS:: RSTAGE
          DOUBLEPRECISION,DIMENSION(:,:), POINTER,CONTIGUOUS:: GSTAGE
          DOUBLEPRECISION,DIMENSION(:,:), POINTER,CONTIGUOUS:: DOBS
C---------DIRECT RUNOFF
          INTEGER,DIMENSION(:,:),CONTIGUOUS,POINTER :: DROIMAP
          REAL,DIMENSION(:,:),CONTIGUOUS, POINTER :: DRORMULT
          REAL,DIMENSION(:,:),CONTIGUOUS, POINTER :: DRORVAL
C-----------DERIVED DATA TYPES
          TYPE(TTABS),     DIMENSION(:),POINTER,CONTIGUOUS :: TABDATA
          TYPE(TOBS),      DIMENSION(:),POINTER,CONTIGUOUS :: OBSDATA
          TYPE(TREACH),    DIMENSION(:),POINTER,CONTIGUOUS :: REACH
          TYPE(TRCHGRP),   DIMENSION(:),POINTER,CONTIGUOUS :: RCHGRP
          TYPE(TJAC),                   POINTER :: JAC
          TYPE(TSWRTIME),  DIMENSION(:),POINTER,CONTIGUOUS :: SWRTIME
          TYPE(TSTABILITY),POINTER :: FROUDE
C-----------PRINT DATA
          INTEGER,POINTER          :: NPMAX
          DOUBLEPRECISION,DIMENSION(:,:), POINTER,CONTIGUOUS :: RSTAGEP
          DOUBLEPRECISION,DIMENSION(:,:), POINTER,CONTIGUOUS :: GSTAGEP
          DOUBLEPRECISION,DIMENSION(:), POINTER,CONTIGUOUS :: DOBSP
        END TYPE GWFSWRTYPE
...
C     SPECIFICATIONS:
C     ------------------------------------------------------------------
      USE GLOBAL,      ONLY:IOUT,NCOL,NROW,NLAY,HNEW,HOLD,PERLEN,SUBLNK
      USE GWFBASMODULE,ONLY:PERTIM
      USE GWFSUBMODULE, ONLY: DVZ,LPFLNK  !SUB-Linkage rth
      USE GWFSWRMODULE
      IMPLICIT NONE
C     + + + DUMMY ARGUMENTS + + +
...
C     + + + INPUT FORMATS + + +
C     + + + OUTPUT FORMATS + + +
2020  FORMAT(/ / 1X,'*** SWR CONVERGENCE FAILURE WITHIN MODFLOW***',/,
...
C     + + + LOCAL DEFINITIONS + + +
        DOUBLEPRECISION :: dt
        REAL::T1temp                                                    !seb TEMP VARIABLE USED FOR PASSING INTO SECNDS cmd
C     + + + FUNCTIONS + + +
C     + + + CODE + + +
        IF ( It.EQ.0 ) THEN
          T1 = SECNDS(0.0)
        ELSE
          T1temp=REAL(T1,kind(T1temp))                                  !seb SECNDS ONLY ACCEPTED REAL AS INPUT
          dt = DBLE( SECNDS(T1temp) )
          Ts = Ts + dt
        END IF
C---------RETURN
        RETURN
