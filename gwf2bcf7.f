      MODULE GWFBCFMODULE
       ...
       INTEGER, SAVE,  POINTER,   DIMENSION(:)    ,CONTIGUOUS ::LAYCON
       INTEGER, SAVE,  POINTER,   DIMENSION(:)    ,CONTIGUOUS ::LAYAVG
       REAL, SAVE,     POINTER,   DIMENSION(:,:,:),CONTIGUOUS ::HY
       REAL, SAVE,     POINTER,   DIMENSION(:,:,:),CONTIGUOUS ::SC1
       REAL, SAVE,     POINTER,   DIMENSION(:,:,:),CONTIGUOUS ::SC2
       REAL, SAVE,     POINTER,   DIMENSION(:,:,:),CONTIGUOUS ::WETDRY
       REAL, SAVE,     POINTER,   DIMENSION(:,:,:),CONTIGUOUS ::CVWD
       REAL, SAVE,     POINTER,   DIMENSION(:)    ,CONTIGUOUS ::TRPY
      TYPE GWFBCFTYPE
       INTEGER, POINTER  ::IBCFCB,IWDFLG,IWETIT,IHDWET
       REAL, POINTER     ::WETFCT
       INTEGER,  POINTER,   DIMENSION(:)    ,CONTIGUOUS ::LAYCON
       INTEGER,  POINTER,   DIMENSION(:)    ,CONTIGUOUS ::LAYAVG
       REAL,     POINTER,   DIMENSION(:,:,:),CONTIGUOUS ::HY
       REAL,     POINTER,   DIMENSION(:,:,:),CONTIGUOUS ::SC1
       REAL,     POINTER,   DIMENSION(:,:,:),CONTIGUOUS ::SC2
       REAL,     POINTER,   DIMENSION(:,:,:),CONTIGUOUS ::WETDRY
       REAL,     POINTER,   DIMENSION(:,:,:),CONTIGUOUS ::CVWD
       REAL,     POINTER,   DIMENSION(:)    ,CONTIGUOUS ::TRPY
      END TYPE
...
C     ------------------------------------------------------------------
     ...
     1                      CC,CV,IFREFM,DEFAULT_KIND,BOTM !swm: added BOTM
      USE GLOBAL,      ONLY:LSTCHK
      USE GWFBASMODULE,ONLY:HDRY
      USE LGRMODULE,   ONLY:HK,VK   !swm: added
      USE GWFBCFMODULE,ONLY:IBCFCB,IWDFLG,IWETIT,IHDWET,WETFCT,
     1                      LAYCON,LAYAVG,HY,SC1,SC2,WETDRY,CVWD,TRPY
C
      REAL,DIMENSION(NCOL,NROW)::BUFFER                                 !seb USED FOR TEMPORARY STORAGE FOR U2DREL READS AND THEN CONVERTED TO DP ARRAYS
...
C
C2A------CHECK IF LGR IS ACTIVE, IF SO STOP.
...
C8C-----READ TRANSMISSIVITY INTO ARRAY CC IF LAYER TYPE IS 0 OR 2.
      ...
      CALL U2DREL(BUFFER,ANAME(2),NROW,NCOL,KK,IN,IOUT)                 !seb MODIFIED TO MATCH PRECISION U2DREL EXPECTS
      CC(:,:,K)=REAL(BUFFER,DEFAULT_KIND)
...
C2E-----MULTIPLIED BY CELL AREA TO CONVERT TO CONDUCTANCE LATER.
  110 IF(K.NE.NLAY) THEN
        CALL U2DREL(BUFFER,ANAME(4),NROW,NCOL,KK,IN,IOUT)                 !seb MODIFIED TO MATCH PRECISION U2DREL EXPECTS
        CV(:,:,K)=REAL(BUFFER,DEFAULT_KIND)
        IF(ILGR .NE. 0) VK(:,:,K) = 0.5*CV(:,:,K)*
     &                  (BOTM(:,:,K-1) - BOTM(:,:,K+1)) !swm: Apprx. VK. 
      ELSE IF(ILGR .NE. 0 .AND. NLAY .GT. 1) THEN
        VK(:,:,K) = VK(:,:,K-1)  !swm: for bottom, use VK of layer above
      END IF        
C
...
C1B-----HORIZONTAL CONDUCTANCES.
      CALL SGWF2BCF7H(KK,KB,KITER,KSTP,KPER,ILGR) !swm: added ILGR
  100 CONTINUE
C
...
C5------RETURN
      RETURN
      END
      SUBROUTINE SGWF2BCF7H(K,KB,KITER,KSTP,KPER,ILGR) !swm: added ILGR
C     ******************************************************************
