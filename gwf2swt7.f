      MODULE GWFSWTMODULE
        ...
        INTEGER, SAVE,    DIMENSION(:),     POINTER,CONTIGUOUS::ISWOCF
        INTEGER, SAVE,    DIMENSION(:),     POINTER,CONTIGUOUS::ISWOCU
        INTEGER, SAVE,    DIMENSION(:),     POINTER,CONTIGUOUS::IFL2
        LOGICAL, SAVE,    DIMENSION(:),     POINTER,CONTIGUOUS::OCLAY2
        INTEGER, SAVE,    DIMENSION(:),     POINTER,CONTIGUOUS::LNWT
C Number of time steps before current stress time step? Nper
        INTEGER, SAVE,    DIMENSION(:),     POINTER,CONTIGUOUS ::NTSSM2
C Output control flags
        LOGICAL, SAVE,    DIMENSION(:,:),   POINTER,CONTIGUOUS ::OCFLG2
C specific gravity of saturated sediments
        REAL,    SAVE,    DIMENSION(:,:),   POINTER,CONTIGUOUS ::SGS
C specific gravity of moist sediments
        REAL,    SAVE,    DIMENSION(:,:),   POINTER,CONTIGUOUS ::SGM
C Preconsolidation stress
        REAL,    SAVE,    DIMENSION(:,:,:), POINTER,CONTIGUOUS ::PCS
C Offset of initial preconsolidation stress from initial effective stress
        REAL,    SAVE,    DIMENSION(:,:,:), POINTER,CONTIGUOUS ::PCSOFF
C Thickness of interbeds in saturated interval
        REAL,    SAVE,    DIMENSION(:,:,:), POINTER,CONTIGUOUS ::THICK
C Recompression index
        REAL,    SAVE,    DIMENSION(:,:,:), POINTER,CONTIGUOUS ::CE
C Compression index
        REAL,    SAVE,    DIMENSION(:,:,:), POINTER,CONTIGUOUS ::CI
C Compaction
        REAL,    SAVE,    DIMENSION(:,:,:), POINTER,CONTIGUOUS ::SUB
C Void ratio
        REAL,    SAVE,    DIMENSION(:,:,:), POINTER,CONTIGUOUS ::VOID
C Effective stress
        REAL,    SAVE,    DIMENSION(:,:,:), POINTER,CONTIGUOUS ::EST
C Effective stress for previous time step
        REAL,    SAVE,    DIMENSION(:,:,:), POINTER,CONTIGUOUS ::ESTOLD
C Geostatic stress
        REAL,    SAVE,    DIMENSION(:,:,:), POINTER,CONTIGUOUS ::GL
C Layer center
        REAL,    SAVE,    DIMENSION(:,:,:), POINTER,CONTIGUOUS ::ZC
C Initial preconsolidation stress
        REAL,    SAVE,    DIMENSION(:,:,:), POINTER,CONTIGUOUS ::PCS0
C Initial geostatic stress
        REAL,    SAVE,    DIMENSION(:,:,:), POINTER,CONTIGUOUS ::GL0
C Initial effective stress
        REAL,    SAVE,    DIMENSION(:,:,:), POINTER,CONTIGUOUS ::EST0
C
      TYPE GWFSWTTYPE
        ...
        INTEGER,    DIMENSION(:),     POINTER,CONTIGUOUS ::ISWOCF
        INTEGER,    DIMENSION(:),     POINTER,CONTIGUOUS ::ISWOCU
        INTEGER,    DIMENSION(:),     POINTER,CONTIGUOUS ::IFL2
        LOGICAL,    DIMENSION(:),     POINTER,CONTIGUOUS ::OCLAY2
        INTEGER,    DIMENSION(:),     POINTER,CONTIGUOUS ::LNWT
        INTEGER,    DIMENSION(:),     POINTER,CONTIGUOUS ::NTSSM2
        LOGICAL,    DIMENSION(:,:),   POINTER,CONTIGUOUS ::OCFLG2
        REAL,       DIMENSION(:,:),   POINTER,CONTIGUOUS ::SGS
        REAL,       DIMENSION(:,:),   POINTER,CONTIGUOUS ::SGM
        REAL,       DIMENSION(:,:,:), POINTER,CONTIGUOUS ::PCS
        REAL,       DIMENSION(:,:,:), POINTER,CONTIGUOUS ::PCSOFF
        REAL,       DIMENSION(:,:,:), POINTER,CONTIGUOUS ::THICK
        REAL,       DIMENSION(:,:,:), POINTER,CONTIGUOUS ::CE
        REAL,       DIMENSION(:,:,:), POINTER,CONTIGUOUS ::CI
        REAL,       DIMENSION(:,:,:), POINTER,CONTIGUOUS ::SUB
        REAL,       DIMENSION(:,:,:), POINTER,CONTIGUOUS ::VOID
        REAL,       DIMENSION(:,:,:), POINTER,CONTIGUOUS ::EST
        REAL,       DIMENSION(:,:,:), POINTER,CONTIGUOUS ::ESTOLD
        REAL,       DIMENSION(:,:,:), POINTER,CONTIGUOUS ::GL
        REAL,       DIMENSION(:,:,:), POINTER,CONTIGUOUS ::ZC
        REAL,       DIMENSION(:,:,:), POINTER,CONTIGUOUS ::PCS0
        REAL,       DIMENSION(:,:,:), POINTER,CONTIGUOUS ::GL0
        REAL,       DIMENSION(:,:,:), POINTER,CONTIGUOUS ::EST0
      END TYPE
...
C5-------PRINT SUBSIDENCE.
     ...
     1   ' sum of compaction values for all systems of interbeds:'
        ENDIF
...
C  Deallocate SWT MEMORY
      USE GWFSWTMODULE
      !
      DEALLOCATE(GWFSWTDAT(IGRID)%ISWTCB)
      DEALLOCATE(GWFSWTDAT(IGRID)%ISWTOC)
      DEALLOCATE(GWFSWTDAT(IGRID)%NSYSTM)
      DEALLOCATE(GWFSWTDAT(IGRID)%ITHK  )
      DEALLOCATE(GWFSWTDAT(IGRID)%IVOID )
      DEALLOCATE(GWFSWTDAT(IGRID)%ISTPCS)
      DEALLOCATE(GWFSWTDAT(IGRID)%ICRCC )
      DEALLOCATE(GWFSWTDAT(IGRID)%IZCFL )
      DEALLOCATE(GWFSWTDAT(IGRID)%IZCFM )
      DEALLOCATE(GWFSWTDAT(IGRID)%IGLFL )
      DEALLOCATE(GWFSWTDAT(IGRID)%IGLFM )
      DEALLOCATE(GWFSWTDAT(IGRID)%IESTFL)
      DEALLOCATE(GWFSWTDAT(IGRID)%IESTFM)
      DEALLOCATE(GWFSWTDAT(IGRID)%IPCSFL)
      DEALLOCATE(GWFSWTDAT(IGRID)%IPCSFM)
      DEALLOCATE(GWFSWTDAT(IGRID)%ISTFL )
      DEALLOCATE(GWFSWTDAT(IGRID)%ISTFM )
      DEALLOCATE(GWFSWTDAT(IGRID)%ISWOCF)
      DEALLOCATE(GWFSWTDAT(IGRID)%ISWOCU)
      DEALLOCATE(GWFSWTDAT(IGRID)%IFL2  )
      DEALLOCATE(GWFSWTDAT(IGRID)%OCLAY2)
      DEALLOCATE(GWFSWTDAT(IGRID)%LNWT  )
      DEALLOCATE(GWFSWTDAT(IGRID)%NTSSM2)
      DEALLOCATE(GWFSWTDAT(IGRID)%OCFLG2)
      DEALLOCATE(GWFSWTDAT(IGRID)%SGS   )
      DEALLOCATE(GWFSWTDAT(IGRID)%SGM   )
      DEALLOCATE(GWFSWTDAT(IGRID)%PCS   )
      DEALLOCATE(GWFSWTDAT(IGRID)%PCSOFF)
      DEALLOCATE(GWFSWTDAT(IGRID)%THICK )
      DEALLOCATE(GWFSWTDAT(IGRID)%CE    )
      DEALLOCATE(GWFSWTDAT(IGRID)%CI    )
      DEALLOCATE(GWFSWTDAT(IGRID)%SUB   )
      DEALLOCATE(GWFSWTDAT(IGRID)%VOID  )
      DEALLOCATE(GWFSWTDAT(IGRID)%EST   )
      DEALLOCATE(GWFSWTDAT(IGRID)%ESTOLD)
      DEALLOCATE(GWFSWTDAT(IGRID)%GL    )
      DEALLOCATE(GWFSWTDAT(IGRID)%ZC    )
      DEALLOCATE(GWFSWTDAT(IGRID)%PCS0  )
      DEALLOCATE(GWFSWTDAT(IGRID)%GL0   )
      DEALLOCATE(GWFSWTDAT(IGRID)%EST0  )
C
