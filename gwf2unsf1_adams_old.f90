...
!  INTEGER,PARAMETER:: IPROU = 97 ! unit for writing profiles, AdamS: chnged to 2 separate files
...
!      WLAYER:
!      LINITW:
!      MAXAL : MAX NUMBER OF ATMOSPHEARIC DATA
!      PERIMP:
...
!      ------
  IF(ITCRIT) THEN
    ...
    END DO
  END IF
  CALL GWF2UNSF1SETMAT(PID) ! modified by AdamS, check if necessary
  IF(WLAYER(PID).GE.0) THEN
...
!      Body
       ALH1 = DLOG10(-HTAB(1))
       !dLOG( LOGARITHM WITH DOUBLE PRECISION
       DLH  =(DLOG10(-HTAB(NTAB))-ALH1)/(NTAB-1)
       DO I=1,NUMNP(PID)! do for all nodes in each profile
        M = MATNUM(I,PID)
...
! modified by AdamS, compilation error in call to FC1,FQ			
        ...
        END IF
 !       open(unit=300,file='con.out')
!        open(unit=301,file='thet.out')
 !       open(unit=302,file='cap.out')
         
        CON(I,PID)=CONI
        CAP(I,PID)=CAPI
        THETA(I,PID)=PAR(1,M)+(THEI-PAR(1,M))
 !       write(300,*)'con',con(i,pid) ,i,pid
 !       write(301,*)'thet',theta(i,pid) ,i,pid
 !       write(302,*)'cap',cap(i,pid),i,pid
